...
- **换设备也能继续工作**：管理员登录其他设备后，可以从服务端恢复完整会话、图片和工作资料；普通用户和游客的聊天正文为了隐私只保存在本地，不提供跨设备同步。
...
**设计动机：** 对话不是一次性的聊天记录，而是不断修订的工作过程。传统“删除重来”会丢掉后续上下文；只提供重新生成又不能比较模型差异。当模型长文本输出因 token 上限截断或提前停止时，用户也需要能够原地继续补全，而不是重新粘贴 prompt 重跑整段长文。

**具体实现：** 输入框支持 `Enter` 发送、`Shift + Enter` 换行；消息悬停后可复制、重新生成、**继续生成**（`▶️` 快捷操作，在未输出完的内容末尾无缝衔接续写后续内容）、编辑、建立分支或单条删除。助手消息支持用 `@` 收藏模型生成新版本，并可切换最多 8 个回答版本；每个版本保留对应的后续消息链、模型名、图片、思考过程和 usage。

<p align="center">
  <img src="public/assets/light-chat-message-shortcuts.png" alt="Light-Chat 真实对话界面中的消息快捷操作" width="800" />
</p>

真实对话界面示例：用户消息与助手回答旁直接显示复制、重新生成、继续生成（▶️）、编辑、分支和删除等操作按钮，底部输入框保持同一套发送工作流。

### 10. 可复用且受控的自定义角色

<ins><em>把稳定的专业工作方式沉淀成角色卡，并在复用时保持系统提示词的服务端安全边界。</em></ins>

**设计动机：** 很多任务需要长期维持同一套角色要求，例如论文解析、RAG 架构评审或工程方案梳理。反复粘贴系统提示词既容易漂移，也让用户无法分辨“本次回答到底用了哪套规则”；同时，角色往往依赖固定的参考规范、专属图表或知识文档，若每次手动上传附件不仅繁琐，还容易遗漏。

**具体实现：** 自定义角色支持文件夹、拖拽排序、右键管理，以及展开查看并跳转到该角色衍生的对话。**角色卡全面兼容附件设定**，支持关联上传图片与文本/文档文件（TXT、Markdown、PDF、DOC/DOCX、PPT/PPTX 等）：
- **文本与文档类文件：** 系统自动解析提取纯文本，在编译系统提示词时作为结构化参考知识库（`【角色关联参考资料/知识库】`）安全注入服务端上下文，为角色赋予准确专属知识；
- **图片与视觉文件：** 自动保留为多模态图像输入（`image_url`）注入模型，让角色具备长效视觉形象与图文多模态设定。

对已登录账户，系统提示词与附件资料由服务端统一管理与注入，客户端不能绕过校验提交任意 system 消息。游客角色则仅保存在当前浏览器，符合游客直连模式的本地边界。
...
<ins><em>重要对话留在自己的工作台里；换到另一台电脑、临时设备或手机时，只要能打开浏览器，就能继续处理。</em></ins>

**设计动机：** 一次在家里完成的论文解读、方案讨论或资料整理，可能包含很难重新复原的上下文。使用某些 AI 厂商的官方软件或网站，换设备时通常要重新登录，还可能要重新配置网络环境；使用只把资料保存在本机的聊天软件，又会遇到历史记录和附件不方便同步的问题。临时设备尤其不适合安装完整客户端或迁移本地数据。

**具体实现：** 将 Light-Chat 部署在自己的设备或服务器上，再通过安全的公网 HTTPS 映射对外提供访问，管理员可以在不同设备的浏览器中登录并从服务端恢复完整会话、图片和工作资料，不依赖某一台浏览器的本地缓存。手机端可安装 GitHub Releases 附带的 Android WebView APK；首次启动时输入自己的 HTTPS 服务地址并保存，之后地址保存在 App 私有存储，登录态由 HttpOnly Cookie 保留，打开 App 即可快速进入工作台，也可以在连接错误页更换服务地址。普通用户和游客仍遵循本地隐私边界，聊天正文不会因为公网部署自动变成跨设备同步数据。

<p align="center">
  <img src="public/assets/light-chat-screenshot.png" alt="Light-Chat 可在浏览器中继续访问的对话工作台" width="800" />
</p>
...
- **Android WebView 客户端：** 移动端需要更明确的网络与凭据边界；客户端首次启动配置一次 HTTPS 服务地址，之后使用受限导航和 HttpOnly Cookie 持久登录，不保存密码或模型服务密钥，复用 Web 工作台能力而不复制一套业务状态。
...
Android 构建的 Base URL 支持 `.local/base-url` 或 `LIGHT_CHAT_BASE_URL`；若使用通用 APK，也可以首次启动后在 App 内配置部署者自己的 HTTPS 域名，方便发布同一个 APK 给不同部署者使用。
