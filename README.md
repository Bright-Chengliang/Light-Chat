...
常用会话可以一键收藏，收藏列表按最近活动自动排序。收藏对话支持右键管理，并可直接**跳回对应的自定义角色界面**，展开该角色的其他历史对话，继续查看同一角色下的工作记录；也可以从历史消息继续上下文，或者把某个历史消息作为新对话的起点。
...
生成过的图片会进入按用户隔离的媒体库，支持跨页浏览、灯箱预览、悬浮下载、上下文菜单复制，以及**从图片跳回源消息**。图片归属严格按 UID 校验，用户之间无法互相读取。

在图片卡片或灯箱中右键，可以选择跳转到所在对话、取消收藏、复制图片或下载文件。
...
工作流是可配置、可复用的多步骤任务：用户输入可以通过连线依次交给角色卡、临时提示词、合并节点和最终生图节点。管理员在画布中拖拽节点、组织连线并保存流程；普通用户只能运行已启用的工作流。它适合把论文解析、RAG 证据整理、研究笔记生成等重复任务沉淀成固定流程；服务端执行时只注入校验过的角色或系统提示词。
...
对话界面直接提供发送与历史消息操作：输入框支持 `Enter` 发送、`Shift + Enter` 换行；消息悬停后可复制、重新生成、编辑、建立分支或删除。助手消息还支持用 `@` 收藏模型重新生成，并可切换最多 8 个回答版本；回答版本会完整保留后续消息链、模型名、图片、思考过程和 usage。
