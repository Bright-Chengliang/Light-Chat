...
登录页支持免密进入游客模式：游客默认没有积分、也没有系统预置的模型权限，但可以在“设置”中自行配置 OpenAI 兼容 API 服务端点、可选 API 密钥和可用模型列表。游客端点必须是公开的 http/https 地址，密钥只保存在服务端进程内存，模型调用仍由服务端代发，不暴露给浏览器。
...
- 游客免密会话只获得零积分、零预置模型权限的隔离角色；自配端点不能指向本机或内网地址，API 密钥不落盘。
...
- 服务端只保存密码哈希、会话令牌、积分、权限、角色定义、游客端点/模型白名单和媒体索引，不收集聊天正文；游客 API 密钥仅保留在进程内存。
