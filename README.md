...
它不是又一个只能打开单一模型页面、发送几条消息的聊天界面。Light-Chat 把多模型对话、生图与图片编辑、可整理的对话历史、可复用角色和多步骤工作流放进同一个工作台；多人使用时，管理员还能统一管理模型权限与额度，而用户的会话和个人资料仍保留在自己的工作区里。
...
登录页支持免密进入游客模式：游客默认没有积分、也没有系统预置的模型权限，但可以在“设置”中自行配置 OpenAI 兼容 API 服务端点、可选 API 密钥和可用模型列表。游客端点必须是公开的 http/https 地址，密钥使用服务端会话密钥加密后落盘，模型调用仍由服务端代发，不暴露给浏览器。
...
- 游客免密会话只获得零积分、零预置模型权限的隔离角色；自配端点不能指向本机或内网地址，API 密钥使用 AES-256-GCM 加密后落盘。
...
- 服务端只保存密码哈希、会话令牌、积分、权限、角色定义、游客端点/模型白名单、加密的游客 API 密钥和媒体索引，不收集聊天正文。
