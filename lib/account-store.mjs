...
export class AccountStore {
  ...
  async createUser({ username, password, credits = 0, modelGroupId = null, extraModels = [], apiKey = null }) {
    ...
    const now = new Date().toISOString();
    const user = { uid, username: normalizedUsername, passwordHash: await hashPassword(normalizedPassword), role: 'user', disabled: false, credits, usagePoints: 0, chatCalls: 0, imageCalls: 0, modelGroupId, extraModels: this.validateModelIds(extraModels), apiKey: normalizedApiKey, createdAt: now, updatedAt: now };
