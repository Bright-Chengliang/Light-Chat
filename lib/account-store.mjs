...
export class AccountStore {
  ...
  validateData(parsed) {
    ...
    const users = parsed.users.map((user) => {
      ...
      const apiKey = typeof user.apiKey === 'string' && user.apiKey.trim().length > 0 ? user.apiKey.trim() : null;
      const defaultModel = typeof user.defaultModel === 'string' && user.defaultModel.trim().length > 0 ? user.defaultModel.trim() : null;
    ...
    const defaultApiKey = typeof parsed.defaultApiKey === 'string' && parsed.defaultApiKey.trim().length > 0 ? parsed.defaultApiKey.trim() : null;
    return { version: 3, revision: Number.isSafeInteger(parsed.revision) ? parsed.revision : 1, nextUserSequence, defaultApiKey, users, modelGroups };
  }

  validateApiKey(value) {
    if (value === undefined || value === null || value === '') return null;
    if (typeof value !== 'string') throw publicError(400, 'API Key 格式无效', 'INVALID_API_KEY');
    const trimmed = value.trim();
    if (trimmed.length === 0) return null;
    if (trimmed.length > 512 || /[\u0000-\u001f\u007f]/.test(trimmed)) {
      throw publicError(400, 'API Key 长度不能超过 512 字符，且不能包含控制字符', 'INVALID_API_KEY');
    }
    return trimmed;
  }

  validateDefaultModel(value) {
    if (value === undefined || value === null || value === '') return null;
    if (typeof value !== 'string') throw publicError(400, '默认模型格式无效', 'INVALID_DEFAULT_MODEL');
    const trimmed = value.trim();
    if (trimmed.length === 0) return null;
    if (trimmed.length > 200 || /[\u0000-\u001f\u007f]/.test(trimmed)) {
      throw publicError(400, '默认模型标识无效', 'INVALID_DEFAULT_MODEL');
    }
    return trimmed;
  }
  ...
  publicUser(user, { forAdmin = false } = {}) {
    ...
    if (forAdmin) {
      result.apiKey = user.apiKey || null;
      result.hasCustomApiKey = Boolean(user.apiKey);
    }
    return result;
  }
  ...
  listUsers({ forAdmin = false } = {}) { return this.data.users.map((user) => this.publicUser(user, { forAdmin })); }
  listModelGroups() { return this.data.modelGroups.map((group) => ({ id: group.id, name: group.name, modelIds: [...group.modelIds] })); }

  getDefaultApiKey() { return this.data.defaultApiKey || null; }

  async setDefaultApiKey(apiKey) {
    const normalized = this.validateApiKey(apiKey);
    this.data.defaultApiKey = normalized;
    this.bumpRevision();
    await this.persist();
    return this.data.defaultApiKey;
  }

  async setUserApiKey(uid, apiKey) {
    const user = this.findUser(uid);
    if (!user) throw publicError(404, '用户不存在', 'USER_NOT_FOUND');
    const normalized = this.validateApiKey(apiKey);
    user.apiKey = normalized;
    user.updatedAt = new Date().toISOString();
    this.bumpRevision();
    await this.persist();
    return this.publicUser(user, { forAdmin: true });
  }
  ...
  userDefaultModel(uid) {
    const user = this.findUser(uid);
    return user?.defaultModel || null;
  }
  ...
  async createUser({ username, password, credits = 0, modelGroupId = null, extraModels = [], apiKey = null, defaultModel = null }) {
    ...
    const normalizedDefaultModel = this.validateDefaultModel(defaultModel);
    ...
    const validatedExtraModels = this.validateModelIds(extraModels);
    if (normalizedDefaultModel !== null) {
      const group = modelGroupId ? this.data.modelGroups.find((item) => item.id === modelGroupId) : null;
      const effectiveAllowed = new Set([...(group?.modelIds || []), ...validatedExtraModels]);
      if (!effectiveAllowed.has(normalizedDefaultModel)) throw publicError(400, '默认模型必须属于该用户的可用模型', 'DEFAULT_MODEL_NOT_ALLOWED');
    }
    ...
    const now = new Date().toISOString();
    const user = { uid, username: normalizedUsername, passwordHash: await hashPassword(normalizedPassword), role: 'user', disabled: false, credits, usagePoints: 0, chatCalls: 0, imageCalls: 0, modelGroupId, extraModels: validatedExtraModels, apiKey: normalizedApiKey, defaultModel: normalizedDefaultModel, createdAt: now, updatedAt: now };
    this.data.nextUserSequence = sequence; this.data.users.push(user); this.bumpRevision(); await this.persist(); return this.publicUser(user, { forAdmin: true });
  }
  ...
  async saveModelGroups(groups, allowedModels) {
    ...
    for (const user of this.data.users) {
      if (user.modelGroupId && !ids.has(user.modelGroupId)) user.modelGroupId = null;
      if (user.defaultModel) {
        const userGroup = user.modelGroupId ? this.data.modelGroups.find((g) => g.id === user.modelGroupId) : null;
        const effectiveAllowed = new Set([...(userGroup?.modelIds || []), ...user.extraModels]);
        if (!effectiveAllowed.has(user.defaultModel)) user.defaultModel = null;
      }
    }
    this.bumpRevision(); await this.persist(); return this.listModelGroups();
  }

  async setUserModelAccess(uid, { modelGroupId = null, extraModels = [], defaultModel = null }, allowedModels) {
    ...
    if (normalizedDefaultModel !== null) {
      const group = modelGroupId ? this.data.modelGroups.find((item) => item.id === modelGroupId) : null;
      const effectiveAllowed = new Set([...(group?.modelIds || []), ...normalizedExtraModels]);
      if (!effectiveAllowed.has(normalizedDefaultModel)) throw publicError(400, '默认模型必须属于该用户的可用模型', 'DEFAULT_MODEL_NOT_ALLOWED');
    }
    user.modelGroupId = modelGroupId;
    user.extraModels = normalizedExtraModels;
    user.defaultModel = normalizedDefaultModel;
    user.updatedAt = new Date().toISOString();
    this.bumpRevision();
    await this.persist();
    return this.publicUser(user, { forAdmin: true });
