...
const MAX_MEDIA_PAGE_SIZE = 50;

const FILE_TYPES = Object.freeze({
  // Android file pickers commonly label plain text as octet-stream. Content
  // validation below still rejects binary payloads before storage.
...
function mediaPage(records, { page = 1, limit = MAX_MEDIA_PAGE_SIZE } = {}) {
  const pageSize = Math.max(1, Math.min(MAX_MEDIA_PAGE_SIZE, Number.parseInt(limit, 10) || MAX_MEDIA_PAGE_SIZE));
  const total = records.length;
  const totalPages = Math.max(1, Math.ceil(total / pageSize));
  const requestedPage = Math.max(1, Number.parseInt(page, 10) || 1);
  const currentPage = Math.min(requestedPage, totalPages);
  const start = (currentPage - 1) * pageSize;
  return { records: records.slice(start, start + pageSize), page: currentPage, pageSize, total, totalPages };
}
...
export class MediaStore {
  ...
  async initialize() {
    ...
    try {
      ...
      if (parsed?.version !== 1 || !Array.isArray(parsed.records)) throw new Error('invalid media index');
      for (const value of parsed.records) {
        ...
        // Only uploads abandoned before they are sent expire. Once a file is used
        // in a conversation or generated by a model, disk capacity is its limit.
        const expiresAt = pending
          ? (Number.isFinite(value.expiresAt) ? value.expiresAt : (value.createdAt || Date.now()) + UPLOAD_TTL_MS)
          : null;
  ...
  async recoverOrphanImages() {
    ...
    for (const name of names) {
      ...
      try {
        ...
        this.records.set(id, { id, path, sessionId: this.defaultOwnerId, kind: 'output', alt: '恢复的历史图片', size: buffer.length, createdAt, expiresAt: null, ...raster, isImage: true, fileName: '' });
  ...
  async save(buffer, { sessionId, kind = 'upload', alt = '' }) {
    ...
    const record = {
      ...
      size: buffer.length,
      createdAt: Date.now(),
      expiresAt: kind === 'upload' ? Date.now() + UPLOAD_TTL_MS : null,
      pending: kind === 'upload',
      ...details,
      isImage: true,
      fileName: '',
  ...
  async saveUpload(buffer, { sessionId, fileName, declaredType, pending = true, expiresAt = undefined } = {}) {
    ...
    const effectiveExpires = isPending ? (expiresAt !== undefined ? expiresAt : Date.now() + UPLOAD_TTL_MS) : null;
    const record = {
      id,
      path,
      sessionId,
  ...
  listRecent(sessionId, options = {}) {
    const page = mediaPage([...this.records.values()]
      .filter((record) => record.sessionId === sessionId && !isExpired(record))
      .sort((left, right) => right.createdAt - left.createdAt), options);
    return { ...page, files: page.records.map((record) => this.publicRecord(record)) };
  }

  recentImageNeighbors(sessionId, id) {
    const images = [...this.records.values()]
      .filter((record) => record.sessionId === sessionId && record.isImage === true && !isExpired(record))
      .sort((left, right) => right.createdAt - left.createdAt);
    const index = images.findIndex((record) => record.id === id);
    if (index < 0) return null;
    return {
      previous: images[index - 1] ? this.publicRecord(images[index - 1]) : null,
      next: images[index + 1] ? this.publicRecord(images[index + 1]) : null,
      position: index + 1,
      total: images.length,
    };
  }

  listOwned(ids, sessionId, options = {}) {
    const requested = Array.isArray(ids) ? ids : [];
    const page = mediaPage(requested.map((id) => this.get(id, sessionId)).filter(Boolean), options);
    return { ...page, files: page.records.map((record) => this.publicRecord(record)) };
  }
  ...
  get(id, sessionId, { kind } = {}) {
    ...
    if (!record || isExpired(record) || record.sessionId !== sessionId) return null;
  ...
  countUploads(sessionId) {
    ...
    for (const record of this.records.values()) {
      if (record.sessionId === sessionId && record.kind === 'upload' && record.pending === true && !isExpired(record)) {
        count += 1;
        bytes += record.size;
      }
    }
