...
function imageGatewaySize(modelId, size) {
  // NewAPI maps its OpenAI Images `size` field to Gemini's aspect ratio. Send
  // the canonical ratio so 4:3/3:4 and the near-ratio pixel presets cannot
  // silently fall back to a square image in that adapter.
  return geminiFlashImageAspectRatio(modelId, size) || size;
}
...
export class NewApiClient {
  ...
  async generateImages({ model, prompt, size, count, quality, forceTool = false, signal }) {
    ...
    try {
      const response = await this.fetch(`${this.baseUrl}/images/generations`, {
        ...
        body: JSON.stringify({ model, prompt, n: count, size: imageGatewaySize(model, size), quality, response_format: 'b64_json' }),
        redirect: 'error',
        signal: linked.signal,
      });
      if (!response.ok) {
