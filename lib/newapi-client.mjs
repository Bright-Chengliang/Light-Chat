...
function geminiFlashImageAspectRatio(modelId, size) {
  if (String(modelId || '').trim().toLowerCase() !== 'gemini-3.1-flash-image') return '';
  return {
    '1024x1024': '1:1',
    '1536x1024': '3:2',
    '1536x1152': '4:3',
...
function geminiFlashImageExtraBody(modelId, size) {
  const aspectRatio = geminiFlashImageAspectRatio(modelId, size);
  return aspectRatio ? { google: { image_config: { aspect_ratio: aspectRatio } } } : undefined;
}
...
function accumulateToolCalls(target, deltaToolCalls) {
  if (!Array.isArray(deltaToolCalls)) return;
  for (const tc of deltaToolCalls) {
    const idx = Number.isInteger(tc.index) ? tc.index : target.length;
    if (!target[idx]) {
      target[idx] = {
        id: tc.id || '',
      ...
      if (tc.id) target[idx].id = tc.id;
      if (tc.function?.name) target[idx].function.name += tc.function.name;
      if (tc.function?.arguments) target[idx].function.arguments += tc.function.arguments;
    }
  }
}
...
export class NewApiClient {
  ...
  authHeaders(apiKey = this.apiKey) {
  ...
  async requireModel(modelId, mode, signal, { allowImageModeOverride = false } = {}) {
    if (typeof modelId !== 'string') throw publicError(400, '请选择模型', 'MODEL_REQUIRED');
    let models = await this.listModels({ signal });
    const matchesMode = (entry) => entry.id === modelId && (entry.modes.includes(mode) || (allowImageModeOverride && mode === 'image'));
    let model = models.find(matchesMode);
    if (!model) {
      models = await this.listModels({ force: true, signal });
      model = models.find(matchesMode);
    }
    if (!model) throw publicError(400, '所选模型不可用，请刷新模型列表', 'MODEL_NOT_ALLOWED');
    return model;
  }

  async chat({ model, messages, tools, toolChoice, temperature, maxTokens, imageSize, stream = true, onDelta = () => {}, onReasoning = () => {}, onToolCall = () => {}, signal }) {
    ...
    try {
      const response = await this.fetch(`${this.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          ...(Array.isArray(tools) && tools.length > 0 ? { tools, ...(toolChoice ? { tool_choice: toolChoice } : {}) } : {}),
          ...(temperature === undefined ? {} : { temperature }),
          ...(maxTokens === undefined ? {} : { max_tokens: maxTokens }),
          // The 3002 Gemini relay uses the OpenAI-compatible size field for
          // image canvas selection. Keep the provider-specific configuration
          // too, because direct Google-compatible channels use that form.
          ...(geminiFlashChat && imageSize ? { size: imageSize } : {}),
          ...(geminiFlashImageExtraBody(model, imageSize) ? { extra_body: geminiFlashImageExtraBody(model, imageSize) } : {}),
        }),
        redirect: 'error',
        signal: linked.signal,
      });
      ...
      if (!contentType.includes('text/event-stream')) {
        ...
        const rawToolCalls = data?.choices?.[0]?.message?.tool_calls || data?.choices?.[0]?.delta?.tool_calls;
        if (Array.isArray(rawToolCalls)) accumulateToolCalls(toolCalls, rawToolCalls);
        ...
        if (stream) {
          ...
          const finalCalls = toolCalls.filter(Boolean);
          if (finalCalls.length > 0) {
            linked.markStarted();
            onToolCall(finalCalls);
          }
        }
      } else {
        ...
        const processLine = (line) => {
          ...
          const rawToolCalls = frame?.choices?.[0]?.delta?.tool_calls || frame?.choices?.[0]?.message?.tool_calls;
          if (Array.isArray(rawToolCalls)) accumulateToolCalls(toolCalls, rawToolCalls);
          text += extracted.text;
          reasoning += extracted.reasoning;
      ...
      if (!markdown.text && images.length === 0 && finalToolCalls.length === 0) throw publicError(502, '模型没有返回可读内容', 'EMPTY_UPSTREAM_RESPONSE');
      return { text: markdown.text, reasoning: reasoning.trim(), images, usage, toolCalls: finalToolCalls };
    } catch (error) {
      if (error?.code) throw error;
  ...
  async generateImages({ model, prompt, size, count, quality, forceTool = false, allowImageModeOverride = false, signal }) {
    await this.requireModel(model, 'image', signal, { allowImageModeOverride });
    if (usesResponsesImageRoute(model)) {
      return this.generateImagesViaResponses({ model, prompt, size, count, quality, forceTool, signal });
    }
  ...
  async generateImagesViaResponses({ model, prompt, size, count, quality, references = [], mask, forceTool = false, signal }) {
    ...
    try {
      const response = await this.fetch(`${this.baseUrl}/responses`, {
        ...
        body: JSON.stringify({
          ...
          tools: [tool],
          ...(forceTool ? { tool_choice: 'required' } : {}),
          stream: false,
          store: false,
        }),
        redirect: 'error',
