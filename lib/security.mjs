...
export const PASSWORD_LIMITS = Object.freeze({ min: 9, max: 128 });
...
export function validatePassword(value) {
  if (
    ...
    throw publicError(400, '密码长度需为 9–128 位');
  }
  return value;
}
...
export function isSafePublicUrl(value) {
  ...
  if (/\.(local|internal|lan|corp|home|arpa)$/i.test(hostname)) return false;
  ...
  if (ipVersion === 4) {
    ...
    if (/^(fc|fd)/i.test(hostname)) return false;
    if (/^fe[89ab]/i.test(hostname)) return false;
  }
  return true;
}
