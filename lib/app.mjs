...
export async function createChatApp({
  ...
  try {
    ...
    adminConversationData = { version: 1, folders, conversations };
  } catch {
    adminConversationData = { version: 1, folders: [], conversations: [] };
  ...
  async function mergeAndSaveAdminConversations(incoming, incomingFolders) {
    const task = adminConversationQueue.then(async () => {
      ...
      adminConversationData = await adminConversationStore.save({ version: 1, folders, conversations: merged });
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && (pathname === '/api/preferences' || pathname === '/api/settings')) {
        ...
        const defaultModel = accountStore.userDefaultModel(found.user.uid);
        if (defaultModel) {
          let groups = userPreferences.favoriteGroups || [];
          if (!groups.length && ['user', 'guest'].includes(found.user.role) && availableModels.length > 0 && availableModels.length <= 20) {
            groups = automaticFavoritesForModels(availableModels);
          }
          if (groups.length > 0) {
            const chatGroup = groups.find((g) => g.items.some((it) => it.mode === 'chat'));
            if (chatGroup) {
              const existingIdx = chatGroup.items.findIndex((it) => (it.modelId || it.model) === defaultModel);
              if (existingIdx > 0) {
                const [item] = chatGroup.items.splice(existingIdx, 1);
                chatGroup.items.unshift(item);
              } else if (existingIdx === -1) {
                chatGroup.items.unshift({ modelId: defaultModel, model: defaultModel, mode: 'chat', label: defaultModel });
              }
            }
          }
          userPreferences = {
            ...
            selected: { modelId: defaultModel, mode: 'chat' },
          };
        } else if (['user', 'guest'].includes(found.user.role) && availableModels.length > 0 && availableModels.length <= 20) {
      ...
      if (req.method === 'PUT' && pathname === '/api/conversations') {
        ...
        const saved = await mergeAndSaveAdminConversations(incoming, incomingFolders);
