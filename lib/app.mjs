...
import { GuestStore, GUEST_UID, GUEST_USERNAME, guestPublicUser, normalizeGuestEndpoint } from './guest-store.mjs';
...
import { EMPTY_ROLE_LIBRARY, compileRoleSystemPrompt, findRole, validateRoleLibrary } from './role-store.mjs';
...
const MAX_CONTEXT_IMAGES = 1000;
const MAX_CONTEXT_MEDIA_BYTES = 100 * 1024 * 1024;
...
const MAX_ADMIN_HISTORY_MESSAGES = 1000;
const MAX_ADMIN_HISTORY_ATTACHMENTS = 1000;
...
const OPC_PROXY_PREFIX = '/opc';
const LEARNING_PROXY_PREFIX = '/learning';
const LEARNING_CLIENT_HEADER = 'x-light-chat-learning-client';
const LEARNING_CLIENT_HEADER_VALUE = '1';
const ADMIN_PROXY_UPSTREAM_HOST = '127.0.0.1';
const OPC_UPSTREAM_PORT = 8765;
const LEARNING_UPSTREAM_PORT = 3021;
...
function publicFailure(error) {
  ...
  if (status >= 500) {
    ...
    return { status, error: error?.publicMessage || messages[status] || '服务暂时不可用', code: error?.code || 'SERVER_ERROR' };
  }
...
function requireSameOrigin(req, sessionManager) {
  const host = String(req.headers.host || '').toLowerCase();
  const origin = req.headers.origin;
  const scheme = sessionManager.isHttps(req) ? 'https' : 'http';
  if (typeof origin !== 'string' || origin !== `${scheme}://${host}`) {
...
function validateFavoriteItems(value, models, maxItems = 20) {
  ...
  return value.map((entry) => {
    ...
    // `/models` metadata is not consistent across OpenAI-compatible relays.
    // A model explicitly placed in a user's image favorite group is an
    // intentional mode override, so do not silently reject it just because
    // capability inference only reported the chat mode.
    if (!model || (entry.mode !== 'image' && !model.modes.includes(entry.mode))) {
...
function sanitizeModelContextLimits(value, models) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  const allowed = new Set(models.map((model) => model.id));
  return Object.fromEntries(Object.entries(value).filter(([modelId, limit]) => allowed.has(modelId) && Number.isInteger(limit) && limit >= 1024 && limit <= MAX_CONFIGURED_CONTEXT_TOKENS));
}

function validateFavoriteMediaIds(value) {
  if (value === undefined) return [];
  if (!Array.isArray(value)) throw publicError(400, '收藏图片列表无效', 'INVALID_FAVORITE_MEDIA');
  const ids = [];
  const seen = new Set();
...
function defaultConversationTitleModel(models) {
  return models.find((model) => model.id === CONVERSATION_TITLE_MODEL && model.modes.includes('chat'))?.id
    || models.find((model) => model.modes.includes('chat'))?.id
    || '';
}

function validateConversationTitleModel(value, models, { fallback = false } = {}) {
  if (value === undefined || value === null || value === '') return defaultConversationTitleModel(models);
  if (typeof value !== 'string' || !value.trim()) throw publicError(400, '会话标题生成模型无效', 'INVALID_CONVERSATION_TITLE_MODEL');
  const modelId = value.trim();
  if (models.some((model) => model.id === modelId && model.modes.includes('chat'))) return modelId;
  if (fallback) return defaultConversationTitleModel(models);
  throw publicError(400, '会话标题生成模型不可用', 'MODEL_NOT_ALLOWED');
}

function validatePreferences(input, models, options = {}) {
  ...
  return {
    ...
    favoriteGroups,
    selected,
    modelContextLimits: validateModelContextLimits(input.modelContextLimits, models),
    favoriteMediaIds: validateFavoriteMediaIds(input.favoriteMediaIds),
    conversationTitleModel: validateConversationTitleModel(input.conversationTitleModel, models, { fallback: options.fallbackConversationTitleModel === true }),
    readingMode: ['fluid', 'classic'].includes(input.readingMode) ? input.readingMode : (options.existingPreferences?.readingMode || 'classic'),
    stream: typeof input.stream === 'boolean' ? input.stream : (options.existingPreferences?.stream !== undefined ? options.existingPreferences.stream : true),
  };
}

function preferencesPayload(preferences, models = []) {
  return {
    favoriteGroups: preferences.favoriteGroups,
    selected: preferences.selected,
    modelContextLimits: sanitizeModelContextLimits(preferences.modelContextLimits || {}, models),
    favoriteMediaIds: preferences.favoriteMediaIds || [],
    conversationTitleModel: preferences.conversationTitleModel || CONVERSATION_TITLE_MODEL,
    readingMode: preferences.readingMode === 'fluid' ? 'fluid' : 'classic',
...
function isLegacyAutomaticFavorites(groups) {
...
function normalizeStoredConversation(value) {
  ...
  return {
    ...
    favoritedAt: Number.isFinite(value?.favoritedAt) && value.favoritedAt > 0 ? Math.floor(value.favoritedAt) : null,
...
const MAX_ADMIN_CONVERSATION_FOLDERS = 30;

function normalizeStoredFolder(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const id = typeof value.id === 'string' && /^[A-Za-z0-9_-]{3,64}$/.test(value.id) ? value.id : '';
  const name = typeof value.name === 'string' ? value.name.trim().slice(0, 40) : '';
  if (!id || !name) return null;
  return { id, name };
}

function normalizeStoredFolders(value) {
  if (!Array.isArray(value)) return [];
  const ids = new Set();
  const folders = [];
  for (const item of value) {
    const folder = normalizeStoredFolder(item);
    if (!folder || ids.has(folder.id)) continue;
    ids.add(folder.id);
    folders.push(folder);
    if (folders.length >= MAX_ADMIN_CONVERSATION_FOLDERS) break;
  }
  return folders;
}

function mergeFolderLists(existingFolders = [], incomingFolders = [], referencedIds = new Set()) {
  ...
  if (Array.isArray(incomingFolders) && incomingFolders.length > 0) {
    for (const raw of incomingFolders) {
      const item = normalizeStoredFolder(raw);
      if (!item || processedIds.has(item.id)) continue;
      processedIds.add(item.id);
      const existing = existingMap.get(item.id);
      if (existing && item.name === '未命名文件夹' && existing.name && existing.name !== '未命名文件夹') {
        merged.push({ id: item.id, name: existing.name });
      } else {
        merged.push(item);
      }
    }
  ...
  for (const existing of existingFolders || []) {
    if (!processedIds.has(existing.id)) {
      if (referencedIds.has(existing.id) || !incomingFolders || incomingFolders.length === 0) {
  ...
  for (const refId of referencedIds) {
    if (!processedIds.has(refId)) {
      ...
      merged.push({ id: refId, name: '未命名文件夹' });
    }
  }

  return merged.slice(0, MAX_ADMIN_CONVERSATION_FOLDERS);
}

function ensureReferencedFolders(folders, conversations) {
  const folderMap = new Map(folders.map((folder) => [folder.id, folder]));
  for (const conversation of conversations) {
    if (conversation?.folderId && !folderMap.has(conversation.folderId)) {
      const folder = { id: conversation.folderId, name: '未命名文件夹' };
      folders.push(folder);
      folderMap.set(conversation.folderId, folder);
      if (folders.length >= MAX_ADMIN_CONVERSATION_FOLDERS) break;
    }
  }
  return folders;
...
function mergeAdminConversations(serverConversations, incomingConversations, deletedIds = new Set()) {
  const deletedSet = deletedIds instanceof Set ? deletedIds : new Set(deletedIds || []);
  const merged = new Map(
    serverConversations
      .filter((conversation) => !deletedSet.has(conversation.id))
      .map((conversation) => [conversation.id, conversation])
  );
  for (const conversation of incomingConversations) {
    if (deletedSet.has(conversation.id)) continue;
...
async function validateConversation(messages, mediaStore, pdfTextExtractor, sessionId, systemPrompt = '', contextLimit = DEFAULT_CONTEXT_TOKENS, role = null) {
  ...
  for (const message of messages) {
    ...
    if (message.role === 'tool') {
      if (typeof message.tool_call_id !== 'string' || !message.tool_call_id.trim()) {
        throw publicError(400, '工具结果缺少调用标识', 'INVALID_TOOL_CALL_ID');
      }
    ...
    normalized.push({
      ...
      toolCallId,
      pdfTexts: new Map(),
      estimatedTokens: estimateTextTokens(content) + (toolCalls ? estimateTextTokens(JSON.stringify(toolCalls)) : 0),
    });
  }
  const last = normalized.at(-1);
  if (last.role !== 'user' && last.role !== 'tool') throw publicError(400, '最后一条消息必须来自用户或工具', 'INVALID_MESSAGES');
  ...
  if (attachmentCount > MAX_ATTACHMENTS) throw publicError(400, `单次请求最多携带 ${MAX_ATTACHMENTS} 个附件`, 'TOO_MANY_ATTACHMENTS');
  ...
  while (retainedStart < normalized.length - 1 && (normalized[retainedStart].role === 'assistant' || normalized[retainedStart].role === 'tool')) retainedStart += 1;

  const roleImageParts = [];
  if (role && Array.isArray(role.attachments)) {
    for (const att of role.attachments) {
      if (!att || !att.isImage) continue;
      let dataUri = '';
      if (typeof att.url === 'string' && att.url.startsWith('data:image/')) {
        dataUri = att.url;
      } else if (typeof att.url === 'string' && (att.url.startsWith('http://') || att.url.startsWith('https://'))) {
        dataUri = att.url;
      } else if (typeof att.url === 'string') {
        const mediaMatch = /\/api\/media\/([a-zA-Z0-9_-]+)/.exec(att.url);
        if (mediaMatch) {
          try {
            const mediaId = mediaMatch[1];
            const { record, buffer } = await mediaStore.read(mediaId, sessionId);
            if (record && buffer) {
              dataUri = `data:${record.mimeType};base64,${buffer.toString('base64')}`;
            }
          } catch {
            try {
              const { record, buffer } = await mediaStore.read(mediaMatch[1], '00000');
              if (record && buffer) {
                dataUri = `data:${record.mimeType};base64,${buffer.toString('base64')}`;
              }
            } catch {}
      ...
      if (dataUri) {
        roleImageParts.push({
          ...
          fileName: att.fileName || att.name || '设定图片',
        });
      }
    }
  }
  ...
  let roleImagesInjected = false;
  const upstream = systemPrompt ? [{ role: 'system', content: systemPrompt }] : [];
  for (const message of normalized.slice(retainedStart)) {
    const { content, ids, imageIds, toolCalls, toolCallId } = message;
    if (message.role === 'tool') {
      upstream.push({ role: 'tool', tool_call_id: toolCallId, content });
      continue;
    }
    if (message.role === 'assistant' && toolCalls) {
      upstream.push({ role: 'assistant', content: content || null, tool_calls: toolCalls });
      continue;
    }
    retainedAttachmentCount += ids.length;
    retainedContextImageCount += imageIds.length;
    if (retainedAttachmentCount > MAX_ATTACHMENTS) throw publicError(400, `单次请求最多携带 ${MAX_ATTACHMENTS} 个附件`, 'TOO_MANY_ATTACHMENTS');
    if (retainedContextImageCount > MAX_CONTEXT_IMAGES) throw publicError(400, `历史上下文最多携带 ${MAX_CONTEXT_IMAGES} 张图片`, 'TOO_MANY_ATTACHMENTS');
    const shouldInjectRoleImages = roleImageParts.length > 0 && !roleImagesInjected && message.role === 'user';
    if (ids.length === 0 && imageIds.length === 0) {
      if (shouldInjectRoleImages) {
        const parts = [];
        for (const p of roleImageParts) {
          parts.push({ type: 'text', text: `【角色关联视觉设定资料：${p.fileName}】` });
          parts.push({ type: 'image_url', image_url: p.image_url });
        }
        if (content.trim()) parts.push({ type: 'text', text: content });
        upstream.push({ role: message.role, content: parts });
        roleImagesInjected = true;
      } else {
...
export async function createChatApp({
  ...
  guestAllowPrivateEndpoints = false,
  opcPort = OPC_UPSTREAM_PORT,
  learningPort = LEARNING_UPSTREAM_PORT,
  ...
  if (!Number.isInteger(opcPort) || opcPort < 1 || opcPort > 65535) {
    throw new Error('OpenOPC 本地端口配置无效');
  }
  if (!Number.isInteger(learningPort) || learningPort < 1 || learningPort > 65535) {
    throw new Error('学习服务本地端口配置无效');
  }
  ...
  const guestStore = new GuestStore(join(dataDir, 'guest-settings.json'), { allowPrivateEndpoints: guestAllowPrivateEndpoints, secret: sessionSecret });
  await guestStore.initialize();
  if (guestStore.apiKeyValue || guestStore.data?.apiKey) {
    guestStore.apiKey = '';
    guestStore.data.apiKey = null;
    await guestStore.persist();
  }
  const loginLimiter = new LoginLimiter(join(dataDir, 'login-rate-limits.json'));
  await loginLimiter.initialize();
  const preferencesStore = new JsonStore(join(dataDir, 'preferences.json'), { version: 1, favoriteGroups: [], selected: null, modelContextLimits: {}, favoriteMediaIds: [], conversationTitleModel: CONVERSATION_TITLE_MODEL });
  ...
  const adminConversationStore = new JsonStore(join(dataDir, 'conversations-00000.json'), { version: 1, folders: [], conversations: [] });
  ...
  if (preferences?.version === 1 && Array.isArray(preferences.favorites) && !Array.isArray(preferences.favoriteGroups)) {
    preferences = await preferencesStore.save({
      ...
      favoriteGroups: preferences.favorites.length
        ...
        : [],
      selected: null,
      modelContextLimits: {}, favoriteMediaIds: [], conversationTitleModel: CONVERSATION_TITLE_MODEL,
    });
  }
  if (preferences?.version !== 1 || !Array.isArray(preferences.favoriteGroups)) {
    preferences = await preferencesStore.save({ version: 1, favoriteGroups: [], selected: null, modelContextLimits: {}, favoriteMediaIds: [], conversationTitleModel: CONVERSATION_TITLE_MODEL });
  } else if (!preferences.modelContextLimits || typeof preferences.modelContextLimits !== 'object' || Array.isArray(preferences.modelContextLimits) || !Array.isArray(preferences.favoriteMediaIds) || typeof preferences.conversationTitleModel !== 'string') {
    preferences = await preferencesStore.save({ ...preferences, modelContextLimits: preferences.modelContextLimits && typeof preferences.modelContextLimits === 'object' && !Array.isArray(preferences.modelContextLimits) ? preferences.modelContextLimits : {}, favoriteMediaIds: Array.isArray(preferences.favoriteMediaIds) ? preferences.favoriteMediaIds : [], conversationTitleModel: typeof preferences.conversationTitleModel === 'string' ? preferences.conversationTitleModel : CONVERSATION_TITLE_MODEL });
  }
  let roleLibrary = validateRoleLibrary(await rolesStore.load());
  let adminConversationData;
  try {
    const loadedData = await adminConversationStore.load();
    const conversations = normalizeAdminConversations(loadedData);
    let folders = normalizeStoredFolders(loadedData?.folders);
    folders = ensureReferencedFolders(folders, conversations);
    const deletedIds = Array.isArray(loadedData?.deletedIds)
      ? loadedData.deletedIds.filter((id) => typeof id === 'string' && id.length > 0 && id.length <= 128)
      : [];
    const deletedSet = new Set(deletedIds);
    const activeConversations = conversations.filter((c) => !deletedSet.has(c.id));
    adminConversationData = { version: 1, folders, conversations: activeConversations, deletedIds };
  } catch {
    adminConversationData = { version: 1, folders: [], conversations: [], deletedIds: [] };
  ...
  async function preferencesFor(uid) {
    ...
    const store = new JsonStore(join(dataDir, `preferences-${uid}.json`), { version: 1, favoriteGroups: [], selected: null, modelContextLimits: {}, favoriteMediaIds: [], conversationTitleModel: CONVERSATION_TITLE_MODEL });
    let value = await store.load();
    if (value?.version !== 1 || !Array.isArray(value.favoriteGroups)) value = await store.save({ version: 1, favoriteGroups: [], selected: null, modelContextLimits: {}, favoriteMediaIds: [], conversationTitleModel: CONVERSATION_TITLE_MODEL });
    if (!value.modelContextLimits || typeof value.modelContextLimits !== 'object' || Array.isArray(value.modelContextLimits) || !Array.isArray(value.favoriteMediaIds) || typeof value.conversationTitleModel !== 'string') value = await store.save({ ...value, modelContextLimits: value.modelContextLimits && typeof value.modelContextLimits === 'object' && !Array.isArray(value.modelContextLimits) ? value.modelContextLimits : {}, favoriteMediaIds: Array.isArray(value.favoriteMediaIds) ? value.favoriteMediaIds : [], conversationTitleModel: typeof value.conversationTitleModel === 'string' ? value.conversationTitleModel : CONVERSATION_TITLE_MODEL });
  ...
  async function mergeAndSaveAdminConversations(incoming, incomingFolders, incomingDeletedIds = []) {
    const task = adminConversationQueue.then(async () => {
      const deletedSet = new Set([
        ...(Array.isArray(adminConversationData.deletedIds) ? adminConversationData.deletedIds : []),
        ...(Array.isArray(incomingDeletedIds) ? incomingDeletedIds : []),
      ]);
      const merged = mergeAdminConversations(adminConversationData.conversations, incoming, deletedSet);
      const referencedFolderIds = new Set(merged.map((c) => c.folderId).filter(Boolean));
      const folders = mergeFolderLists(adminConversationData.folders, incomingFolders, referencedFolderIds);
      await recoverAdminConversationMedia(merged);
      const recentDeletedIds = [...deletedSet].slice(-2000);
      adminConversationData = await adminConversationStore.save({ version: 1, folders, conversations: merged, deletedIds: recentDeletedIds });
      return structuredClone(adminConversationData);
    });
  ...
  async function deleteAdminConversation(conversationId) {
    const task = adminConversationQueue.then(async () => {
      ...
      const conversations = (adminConversationData.conversations || []).filter((c) => c.id !== conversationId);
      const referencedFolderIds = new Set(conversations.map((c) => c.folderId).filter(Boolean));
      const folders = (adminConversationData.folders || []).filter((f) => referencedFolderIds.has(f.id) || true);
      const recentDeletedIds = [...deletedSet].slice(-2000);
      adminConversationData = await adminConversationStore.save({ version: 1, folders, conversations, deletedIds: recentDeletedIds });
      return structuredClone(adminConversationData);
    });
    adminConversationQueue = task.catch(() => {});
    return task;
  ...
  async function clearAllAdminConversations() {
    const task = adminConversationQueue.then(async () => {
      const deletedSet = new Set([
        ...(Array.isArray(adminConversationData.deletedIds) ? adminConversationData.deletedIds : []),
        ...(adminConversationData.conversations || []).map((c) => c.id),
      ]);
      const recentDeletedIds = [...deletedSet].slice(-2000);
      adminConversationData = await adminConversationStore.save({ version: 1, folders: [], conversations: [], deletedIds: recentDeletedIds });
      return structuredClone(adminConversationData);
  ...
  async function readAdminConversations() {
    ...
    return {
      version: 1,
      folders: (adminConversationData.folders || []).map((folder) => structuredClone(folder)),
      conversations: adminConversationData.conversations.map((conversation) => structuredClone(conversation)),
      deletedIds: Array.isArray(adminConversationData.deletedIds) ? [...adminConversationData.deletedIds] : [],
    };
  }
  ...
  function closeSessionEventStream(stream) {
    if (!stream || stream.closed) return;
    stream.closed = true;
    clearInterval(stream.heartbeat);
    const streams = sessionEventStreams.get(stream.uid);
    streams?.delete(stream);
    if (streams?.size === 0) sessionEventStreams.delete(stream.uid);
    if (!stream.res.writableEnded && !stream.res.destroyed) stream.res.end();
  }

  function publishSessionInvalidation(uid, reason = 'session_revoked') {
    for (const stream of [...(sessionEventStreams.get(uid) || [])]) {
      sendSse(stream.res, 'logout', { reason });
      closeSessionEventStream(stream);
    }
  }

  async function invalidateUidSessions(uid, reason = 'session_revoked') {
    publishSessionInvalidation(uid, reason);
    sessionManager.invalidateUid(uid);
    await sessionManager.flush();
  }

  function requireActiveAccount(req) {
    const found = sessionManager.require(req);
    const user = sessionUserFor(found.session.uid);
  ...
  function sessionUserFor(uid) {
    return uid === GUEST_UID ? guestPublicUser() : accountStore.getPublicUser(uid);
  }

  function isActiveSessionUid(uid) {
    return uid === GUEST_UID || accountStore.isActive(uid);
  }
  ...
  function guestApiClient() {
    if (!guestStore.endpoint) {
      throw publicError(400, '请先在设置中配置 API 服务端点', 'GUEST_ENDPOINT_REQUIRED');
    }
    if (
      !guestApi
      || guestApi.baseUrl !== guestStore.endpoint
      || guestApi.apiKey !== guestStore.apiKeyValue
    ) {
      guestApi = new NewApiClient({
        apiKey: guestStore.apiKeyValue,
        baseUrl: guestStore.endpoint,
        fetchImpl,
      });
    }
    return guestApi;
  ...
  function resolveEffectiveApiKey(uid) {
    if (uid === GUEST_UID) return null;
    const user = accountStore.findUser(uid);
    if (user?.apiKey && typeof user.apiKey === 'string' && user.apiKey.trim()) {
      return user.apiKey.trim();
    }
    const defaultKey = accountStore.getDefaultApiKey();
  ...
  function apiClientFor(uid) {
    if (uid === GUEST_UID) return guestApiClient();
    const effectiveKey = resolveEffectiveApiKey(uid);
    if (!effectiveKey) return newApi;
    if (effectiveKey === apiKey && !accountStore.getDefaultApiKey()) {
      return newApi;
    }
    let client = clientCache.get(effectiveKey);
    if (!client) {
      client = new NewApiClient({ apiKey: effectiveKey, baseUrl: newApiBaseUrl, fetchImpl });
      clientCache.set(effectiveKey, client);
    }
    return client;
  }
  ...
  async function modelsForUser(uid, { force = false } = {}) {
    ...
    let models;
    try {
      const client = apiClientFor(uid);
      models = await client.listModels({ force });
    } catch {
      try {
        models = await newApi.listModels({ force: false });
      } catch {
        models = upstreamModels || [];
      }
    }
    const allowed = accountStore.allowedModelIds(uid);
    const filtered = allowed === null ? models : models.filter((model) => allowed.has(model.id));
    const defaultModel = accountStore.userDefaultModel(uid);
    if (defaultModel) {
      const idx = filtered.findIndex((m) => m.id === defaultModel);
      if (idx > 0) {
  ...
  function requireModelAccess(uid, modelId) {
    if (uid === GUEST_UID) {
      throw publicError(410, '游客模式已改为浏览器直连，请从浏览器直接发送模型请求', 'GUEST_DIRECT_ONLY');
    }
    if (!accountStore.canUseModel(uid, modelId)) throw publicError(403, '当前账号没有使用该模型的权限', 'MODEL_ACCESS_DENIED');
  }

  function reserveQuotaFor(user, mode, requestedCost = undefined) {
    if (user.role === 'guest') {
      let settled = false;
      return {
        cost: 0,
    ...
    return accountStore.reserveQuota(user.uid, mode, requestedCost);
  }
  ...
  async function executeWorkflow({ workflow, userPrompt, imageModel, size, quality, userUid, sessionId, signal, onImage = () => {} }) {
    ...
    try {
      ...
      for (const node of executionOrder) {
        ...
        const systemPrompt = node.type === 'role' ? compileRoleSystemPrompt(role) : node.systemPrompt;
        ...
        startedChatNodes += 1;
        const chat = await apiClientFor(userUid).chat({ model: node.model, messages: chatMessages, stream: false, signal });
      ...
      let generated;
      try {
        generated = await apiClientFor(userUid).generateImages({ model: imageModel, prompt: promptCandidates.primary, size, count: 1, quality, forceTool: true, allowImageModeOverride: true, signal });
      } catch (primaryError) {
        if (!promptCandidates.fallback) throw primaryError;
        requirePromptWithinContext(promptCandidates.fallback, imageContextLimit);
        generated = await apiClientFor(userUid).generateImages({ model: imageModel, prompt: promptCandidates.fallback, size, count: 1, quality, forceTool: true, allowImageModeOverride: true, signal });
      }
  ...
  const adminProxies = [
    { prefix: OPC_PROXY_PREFIX, port: opcPort, browserWsPort: OPC_UPSTREAM_PORT, webSocketPaths: new Set(['/ws']), name: 'OPC', code: 'OPC' },
    // Keep the learning app under the authenticated `/learning` origin. The
    // upstream is a standalone Next app, so its document, bundles, styles and
    // Flight payloads need their root-relative requests rewritten here.
    // Do not depend on Service Worker activation: it can remain pending on a
    // first public visit and leaves the user on a loading page indefinitely.
    { prefix: LEARNING_PROXY_PREFIX, port: learningPort, browserWsPort: LEARNING_UPSTREAM_PORT, webSocketPaths: new Set(['/ws', '/_next/webpack-hmr']), upstreamOrigin: `http://${ADMIN_PROXY_UPSTREAM_HOST}:${learningPort}`, name: '学习服务', code: 'LEARNING' },
  ];

  function adminProxyForPathname(pathname) {
    return adminProxies.find((proxy) => pathname === proxy.prefix || pathname.startsWith(`${proxy.prefix}/`)) || null;
  }

  function adminProxyPathFromRequest(req, proxy) {
    const parsed = new URL(req.url, 'http://localhost');
    if (!proxy || !parsed.pathname.startsWith(`${proxy.prefix}/`)) return null;
    return `${parsed.pathname.slice(proxy.prefix.length) || '/'}${parsed.search}`;
  }

  function forwardAdminProxyRequestHeaders(headers, proxy, { websocket = false } = {}) {
    const forwarded = {};
    for (const [name, value] of Object.entries(headers)) {
      const normalized = name.toLowerCase();
      if (
        HOP_BY_HOP_HEADERS.has(normalized)
        || ['host', 'cookie', 'authorization', 'x-csrf-token', 'x-light-chat-learning-client', 'accept-encoding'].includes(normalized)
      ) continue;
      forwarded[name] = value;
    }
    forwarded.host = `${ADMIN_PROXY_UPSTREAM_HOST}:${proxy.port}`;
    if (proxy.upstreamOrigin) forwarded.origin = proxy.upstreamOrigin;
    if (proxy.prefix === LEARNING_PROXY_PREFIX) forwarded['x-forwarded-prefix'] = proxy.prefix;
    if (websocket) {
  ...
  function forwardAdminProxyResponseHeaders(headers, proxy, { bodyChanged = false, websocket = false } = {}) {
    const forwarded = {};
    for (const [name, value] of Object.entries(headers)) {
      const normalized = name.toLowerCase();
      if ((HOP_BY_HOP_HEADERS.has(normalized) && !(websocket && ['connection', 'upgrade'].includes(normalized))) || normalized === 'set-cookie' || normalized === 'content-security-policy') continue;
      if (bodyChanged && normalized === 'content-length') continue;
      if (normalized === 'location' && typeof value === 'string') {
        try {
          const location = new URL(value, `http://${ADMIN_PROXY_UPSTREAM_HOST}:${proxy.port}`);
          if (location.hostname === ADMIN_PROXY_UPSTREAM_HOST && location.port === String(proxy.port)) {
            forwarded[name] = `${proxy.prefix}${location.pathname}${location.search}${location.hash}`;
            continue;
          }
        } catch { /* Preserve an invalid or external Location unchanged. */ }
      }
      forwarded[name] = value;
    }
    return forwarded;
  ...
  function rewriteAdminProxyBrowserBundle(source, proxy) {
    // The local admin tools are standalone SPAs. Rewrite their root API and
    // default WebSocket targets so traffic stays on the authenticated origin.
    const basePathPatched = source;
    return basePathPatched
      .replaceAll('"/api"', `"${proxy.prefix}/api"`)
      .replaceAll("'/api'", `'${proxy.prefix}/api'`)
      .replaceAll('`/api`', `\`${proxy.prefix}/api\``)
      .replaceAll('"/api/', `"${proxy.prefix}/api/`)
      .replaceAll("'/api/", `'${proxy.prefix}/api/`)
      .replaceAll('`/api/', `\`${proxy.prefix}/api/`)
      .replaceAll('"/_next/', `"${proxy.prefix}/_next/`)
      .replaceAll("'/_next/", `'${proxy.prefix}/_next/`)
      .replaceAll('`/_next/', `\`${proxy.prefix}/_next/`)
      .replaceAll(`window.location.hostname}:\${window.location.port||"${proxy.browserWsPort}"}/ws`, `window.location.host}${proxy.prefix}/ws`);
  }

  function rewriteAdminProxyHtml(source, proxy) {
    const rewritten = source
      .replace(/\b(href|src|action)=(['"])\/(?!\/)/gi, `$1=$2${proxy.prefix}/`)
      .replaceAll('"/api"', `"${proxy.prefix}/api"`)
      .replaceAll('\\"/api\\"', `\\"${proxy.prefix}/api\\"`)
      .replaceAll('"/api/', `"${proxy.prefix}/api/`)
      .replaceAll('\\"/api/', `\\"${proxy.prefix}/api/`)
      .replaceAll('"/_next/', `"${proxy.prefix}/_next/`)
      .replaceAll('\\"/_next/', `\\"${proxy.prefix}/_next/`)
      .replaceAll('"/manifest.webmanifest', `"${proxy.prefix}/manifest.webmanifest`)
      .replaceAll('\\"/manifest.webmanifest', `\\"${proxy.prefix}/manifest.webmanifest`)
      .replaceAll('"/app-icons/', `"${proxy.prefix}/app-icons/`)
      .replaceAll('\\"/app-icons/', `\\"${proxy.prefix}/app-icons/`);
    return rewritten.replace(new RegExp(`((?:href|src)=['"]${proxy.prefix.replace('/', '\\/')}\/_next\/[^'"]+)(['"])`, 'gi'), '$1?proxy=1$2');
  }

  function rewriteAdminProxyStyleSheet(source, proxy) {
    return source.replace(/url\((['"]?)\/(?!\/)/gi, `url($1${proxy.prefix}/`);
  }

  function learningBootstrapHtml() {
    return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>正在打开学习工作台</title><script src="${LEARNING_PROXY_PREFIX}/bootstrap.js" defer></script></head><body><p id="learning-status">正在安全打开学习工作台…</p></body></html>`;
  }

  function learningBootstrapScript() {
    const script = [
      '(() => {',
      "  const status = document.getElementById('learning-status');",
      "  if (!('serviceWorker' in navigator)) {",
      "    status.textContent = '当前浏览器不支持学习工作台所需的安全加载能力。';",
      '    return;',
      '  }',
      `  navigator.serviceWorker.register('${LEARNING_PROXY_PREFIX}/service-worker.js', { scope: '/' })`,
      '    .then(() => navigator.serviceWorker.ready)',
      `    .then(() => location.replace('${LEARNING_PROXY_PREFIX}/'))`,
      "    .catch(() => { status.textContent = '学习工作台加载失败，请刷新后重试。'; });",
      '})();',
      '',
    ];
    return script.join('\n');
  ...
  function learningServiceWorkerScript() {
    return [
      `const PREFIX = '${LEARNING_PROXY_PREFIX}';`,
      `const CLIENT_HEADER = '${LEARNING_CLIENT_HEADER}';`,
      `const CLIENT_VALUE = '${LEARNING_CLIENT_HEADER_VALUE}';`,
      '',
      'function learningClient(client) {',
      "  return client && new URL(client.url).pathname.startsWith(PREFIX + '/');",
      '}',
      '',
      'function remap(url) {',
      '  const target = new URL(url);',
      "  if (target.pathname === PREFIX || target.pathname.startsWith(PREFIX + '/')) target.pathname = target.pathname.slice(PREFIX.length) || '/';",
      '  return target;',
      '}',
      '',
      'async function proxiedRequest(request, target) {',
      '  const headers = new Headers(request.headers);',
      '  headers.set(CLIENT_HEADER, CLIENT_VALUE);',
      "  const init = { method: request.method, headers, credentials: 'same-origin', redirect: 'manual' };",
      ...
      "    if (source.origin !== self.location.origin || source.pathname === PREFIX + '/service-worker.js') return fetch(request);",
      '    if (request.headers.get(CLIENT_HEADER) === CLIENT_VALUE) return fetch(request);',
      '    const client = event.clientId ? await self.clients.get(event.clientId) : null;',
      '    if (!learningClient(client)) return fetch(request);',
      '    try { return await fetch(await proxiedRequest(request, remap(source))); }',
      "    catch (error) { return new Response(JSON.stringify({ error: String(error?.message || error), code: 'LEARNING_CLIENT_FETCH_FAILED' }), { status: 502, headers: { 'Content-Type': 'application/json' } }); }",
      '  })());',
      '});',
      '',
    ].join('\n');
  ...
  function setLearningContentSecurityPolicy(res) {
    // Socratopia's production App Router document contains framework bootstrap
    // snippets inline. Keep the policy scoped to the already administrator-
    // gated learning surface; the Light-Chat shell keeps its stricter policy.
    ...
    // setSecurityHeaders() ran before routing and already set a stricter CSP.
    // Replace it rather than append a second policy: browsers enforce both.
  }

  function serveLearningBootstrap(res, fileName) {
    const isServiceWorker = fileName === 'service-worker.js';
    const body = isServiceWorker ? learningServiceWorkerScript() : learningBootstrapScript();
    res.writeHead(200, {
  ...
  async function proxyAdminHttp(req, res, proxy, path) {
    await new Promise((resolve, reject) => {
      const upstream = httpRequest({
        host: ADMIN_PROXY_UPSTREAM_HOST,
        port: proxy.port,
        method: req.method,
        path,
        headers: forwardAdminProxyRequestHeaders(req.headers, proxy),
      }, (upstreamRes) => {
        const contentType = String(upstreamRes.headers['content-type'] || '').toLowerCase();
        const isHtml = req.method !== 'HEAD' && contentType.includes('text/html');
        const isJavaScript = req.method !== 'HEAD' && (contentType.includes('javascript') || contentType.includes('ecmascript'));
        const isStyleSheet = req.method !== 'HEAD' && contentType.includes('text/css');
        const isFlight = req.method !== 'HEAD' && contentType.includes('text/x-component');
        const shouldRewriteContent = proxy.rewriteContent !== false && (isHtml || isJavaScript || isStyleSheet || isFlight);
        if (!shouldRewriteContent) {
          res.writeHead(upstreamRes.statusCode || 502, forwardAdminProxyResponseHeaders(upstreamRes.headers, proxy));
        ...
        upstreamRes.on('data', (chunk) => {
          size += chunk.length;
          if (size > 2 * 1024 * 1024) {
            upstreamRes.destroy(publicError(502, `${proxy.name} 页面响应过大`, `${proxy.code}_RESPONSE_TOO_LARGE`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        upstreamRes.once('end', () => {
          ...
          const body = isJavaScript
            ? rewriteAdminProxyBrowserBundle(source, proxy)
            : isHtml
              ? rewriteAdminProxyHtml(source, proxy)
              : isStyleSheet
                ? rewriteAdminProxyStyleSheet(source, proxy)
                : rewriteAdminProxyHtml(source, proxy);
          const responseHeaders = forwardAdminProxyResponseHeaders(upstreamRes.headers, proxy, { bodyChanged: true });
      ...
      upstream.once('error', () => reject(publicError(502, `${proxy.name} 暂时不可用`, `${proxy.code}_UNAVAILABLE`)));
      req.once('aborted', () => upstream.destroy());
      req.pipe(upstream);
    });
  }

  function rejectAdminProxyUpgrade(socket, status, message) {
    socket.write(`HTTP/1.1 ${status} ${message}\r\nConnection: close\r\nCache-Control: no-store\r\n\r\n`);
    socket.destroy();
  }

  function proxyAdminUpgrade(req, socket, head) {
    let proxy;
    let path;
    try {
      const pathname = new URL(req.url, 'http://localhost').pathname;
      proxy = adminProxyForPathname(pathname);
      path = adminProxyPathFromRequest(req, proxy);
      if (!isAllowedHost(req.headers.host, allowedHosts, port)) throw publicError(421, '请求主机无效', 'HOST_REJECTED');
      if (!proxy || !path || !proxy.webSocketPaths.has(new URL(path, 'http://localhost').pathname)) throw publicError(404, '未找到该接口', 'NOT_FOUND');
      requireAdministrator(req);
      requireSameOrigin(req, sessionManager);
    } catch (error) {
      rejectAdminProxyUpgrade(socket, Number.isInteger(error?.status) ? error.status : 400, publicFailure(error).error);
      return;
    }
    ...
    const upstream = httpRequest({
      ...
      method: 'GET',
      path,
      headers: forwardAdminProxyRequestHeaders(req.headers, proxy, { websocket: true }),
    });
    upstream.once('error', () => rejectAdminProxyUpgrade(socket, 502, `${proxy.name} 暂时不可用`));
    upstream.once('response', (upstreamRes) => {
      const status = upstreamRes.statusCode || 502;
      const headers = forwardAdminProxyResponseHeaders(upstreamRes.headers, proxy, { websocket: true });
      socket.write(`HTTP/1.1 ${status} ${upstreamRes.statusMessage || 'Bad Gateway'}\r\n${Object.entries(headers).map(([name, value]) => `${name}: ${Array.isArray(value) ? value.join(', ') : value}`).join('\r\n')}\r\n\r\n`);
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname === LEARNING_PROXY_PREFIX || pathname.startsWith(`${LEARNING_PROXY_PREFIX}/`)) setLearningContentSecurityPolicy(res);

      const adminProxy = adminProxyForPathname(pathname);
      if (pathname === `${LEARNING_PROXY_PREFIX}/service-worker.js` && (req.method === 'GET' || req.method === 'HEAD')) {
        requireAdministrator(req);
        if (req.method === 'HEAD') {
          res.writeHead(200, { 'Content-Type': 'application/javascript; charset=utf-8', 'Cache-Control': 'no-store', 'Service-Worker-Allowed': '/' });
          res.end();
        } else serveLearningBootstrap(res, 'service-worker.js');
        return;
      }
      if (pathname === `${LEARNING_PROXY_PREFIX}/bootstrap.js` && (req.method === 'GET' || req.method === 'HEAD')) {
        requireAdministrator(req);
        if (req.method === 'HEAD') {
          res.writeHead(200, { 'Content-Type': 'application/javascript; charset=utf-8', 'Cache-Control': 'no-store' });
          res.end();
        } else serveLearningBootstrap(res, 'bootstrap.js');
        return;
      }
      if (req.headers[LEARNING_CLIENT_HEADER] === LEARNING_CLIENT_HEADER_VALUE && pathname === '/') {
        const learningProxy = adminProxies.find((proxy) => proxy.prefix === LEARNING_PROXY_PREFIX);
        requireAdministrator(req);
        setLearningContentSecurityPolicy(res);
        if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method || '')) requireSameOrigin(req, sessionManager);
        await proxyAdminHttp(req, res, learningProxy, `${pathname}${new URL(req.url, 'http://localhost').search}`);
        return;
      }
      if (req.headers[LEARNING_CLIENT_HEADER] === LEARNING_CLIENT_HEADER_VALUE && !pathname.startsWith(`${LEARNING_PROXY_PREFIX}/`)) {
        const learningProxy = adminProxies.find((proxy) => proxy.prefix === LEARNING_PROXY_PREFIX);
        requireAdministrator(req);
        setLearningContentSecurityPolicy(res);
        if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method || '')) requireSameOrigin(req, sessionManager);
      ...
      if (adminProxy && pathname === adminProxy.prefix && (req.method === 'GET' || req.method === 'HEAD')) {
        requireAdministrator(req);
        const requestUrl = new URL(req.url, 'http://localhost');
        res.writeHead(303, { Location: `${adminProxy.prefix}/${requestUrl.search}`, 'Cache-Control': 'no-store' });
        res.end();
        return;
      }
      if (adminProxy && pathname.startsWith(`${adminProxy.prefix}/`)) {
        requireAdministrator(req);
        if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method || '')) requireSameOrigin(req, sessionManager);
        await proxyAdminHttp(req, res, adminProxy, adminProxyPathFromRequest(req, adminProxy));
        return;
      }
      ...
      if ((req.method === 'GET' || req.method === 'HEAD') && ['/', '/index.html', '/login.html'].includes(pathname)) {
        ...
        if (found?.session.authenticated && isActiveSessionUid(found.session.uid)) {
      ...
      if (req.method === 'GET' && (pathname === '/api/session' || pathname === '/api/login/bootstrap')) {
        ...
        let created;
        if (found?.session.authenticated && !isActiveSessionUid(found.session.uid)) { sessionManager.invalidate(found); found = null; }
        if (!found) {
        ...
        const sessionUser = found.session.authenticated ? sessionUserFor(found.session.uid) : null;
        const sessionPayload = {
          ...
          defaultModel: sessionUser?.defaultModel || null,
          csrfToken: found.session.csrfToken,
          csrf: found.session.csrfToken,
          enableWorkspaces: Boolean(enableWorkspaces && sessionUser?.role === 'admin'),
        };
      ...
      if (req.method === 'GET' && pathname === '/api/session/events') {
        ...
        res.flushHeaders?.();
        const stream = { uid: found.user.uid, res, heartbeat: null, closed: false };
        const streams = sessionEventStreams.get(stream.uid) || new Set();
        streams.add(stream); sessionEventStreams.set(stream.uid, streams);
        stream.heartbeat = setInterval(() => { if (!stream.closed) res.write(': keep-alive\n\n'); }, SSE_HEARTBEAT_INTERVAL_MS);
        stream.heartbeat.unref?.();
        res.once('close', () => closeSessionEventStream(stream));
        sendSse(res, 'ready', {});
        return;
      }
      ...
      if (req.method === 'PUT' && pathname === '/api/account') {
        ...
        await invalidateUidSessions(found.user.uid, 'credentials_changed');
        res.setHeader('Set-Cookie', sessionManager.clearCookieHeaders());
        sendJson(res, 200, { authenticated: false, username: changed.username, uid: changed.uid, role: changed.role, credits: changed.credits });
        return;
      }
      ...
      if (req.method === 'GET' && pathname === '/api/guest/settings') {
        const found = requireActiveAccount(req);
        if (found.user.role !== 'guest') throw publicError(403, '仅游客可访问此配置', 'ADMIN_REQUIRED');
        sendJson(res, 200, { direct: true, endpoint: '', hasApiKey: false, allowedModels: [] });
        return;
      }

      if (req.method === 'PUT' && pathname === '/api/guest/settings') {
        const found = requireActiveAccount(req);
        if (found.user.role !== 'guest') throw publicError(403, '仅游客可访问此配置', 'ADMIN_REQUIRED');
        requireSafeMutation(req, sessionManager, found);
        throw publicError(410, '游客模式已改为浏览器直连，请在当前浏览器保存 API 密钥', 'GUEST_DIRECT_ONLY');
      }

      if (req.method === 'POST' && pathname === '/api/guest/models/preview') {
        const found = requireActiveAccount(req);
        if (found.user.role !== 'guest') throw publicError(403, '仅游客可访问此配置', 'ADMIN_REQUIRED');
        throw publicError(410, '游客模式已改为浏览器直连，请在当前浏览器获取模型列表', 'GUEST_DIRECT_ONLY');
      }

      if (req.method === 'GET' && pathname === '/api/admin/users') {
        requireAdministrator(req);
        sendJson(res, 200, {
          users: accountStore.listUsers({ forAdmin: true }),
          total: accountStore.data.users.length,
          revision: accountStore.data.revision,
          defaultApiKey: accountStore.getDefaultApiKey(),
          hasSystemKey: Boolean(apiKey),
        });
        return;
      }

      if (req.method === 'GET' && pathname === '/api/admin/default-api-key') {
        requireAdministrator(req);
        sendJson(res, 200, {
          defaultApiKey: accountStore.getDefaultApiKey(),
          hasSystemKey: Boolean(apiKey),
      ...
      if (req.method === 'PUT' && pathname === '/api/admin/default-api-key') {
        const found = requireAdministrator(req);
        requireSafeMutation(req, sessionManager, found);
        const body = await readJson(req, 8 * 1024);
        const defaultApiKey = await runAdminMutation(async () => {
          const saved = await accountStore.setDefaultApiKey(body?.apiKey ?? body?.defaultApiKey ?? null);
          await auditAdminAction('default-api-key.update', found.user.uid, null, { hasCustom: Boolean(saved) });
          return saved;
        });
        sendJson(res, 200, {
          defaultApiKey,
      ...
      const adminUserMatch = /^\/api\/admin\/users\/(\d{5}|\d{7})(?:\/(status|recharge|model-access|api-key))?$/.exec(pathname);
      if (adminUserMatch) {
        const found = requireAdministrator(req); requireSafeMutation(req, sessionManager, found);
        const [, uid, action] = adminUserMatch;
        if (req.method === 'DELETE' && !action) {
          await runAdminMutation(async () => { await accountStore.deleteUser(uid); await invalidateUidSessions(uid, 'account_deleted'); await auditAdminAction('user.delete', found.user.uid, uid); }); sendJson(res, 200, { deleted: true, uid, revision: accountStore.data.revision }); return;
        }
        if (req.method === 'PUT' && action === 'status') {
          const body = await readJson(req, 8 * 1024); const user = await runAdminMutation(async () => { const updated = await accountStore.setUserDisabled(uid, body?.disabled === true); await invalidateUidSessions(uid, body?.disabled === true ? 'account_disabled' : 'session_reset'); await auditAdminAction(body?.disabled === true ? 'user.disable' : 'user.enable', found.user.uid, uid); return updated; }); sendJson(res, 200, { user, revision: accountStore.data.revision }); return;
        }
        ...
        if (req.method === 'PUT' && action === 'model-access') {
          ...
          const user = await runAdminMutation(async () => {
            const updated = await accountStore.setUserModelAccess(uid, body || {}, models.map((model) => model.id));
            if (updated.defaultModel) {
              const currentPrefs = await preferencesFor(uid);
              const available = await modelsForUser(uid);
              let nextGroups = currentPrefs.favoriteGroups || [];
              if (nextGroups.length > 0) {
                const chatGroup = nextGroups.find((g) => g.items.some((it) => it.mode === 'chat'));
                if (chatGroup) {
                  const existingIdx = chatGroup.items.findIndex((it) => (it.modelId || it.model) === updated.defaultModel);
                  if (existingIdx > 0) {
                    const [item] = chatGroup.items.splice(existingIdx, 1);
                    chatGroup.items.unshift(item);
                  } else if (existingIdx === -1) {
                    chatGroup.items.unshift({ modelId: updated.defaultModel, model: updated.defaultModel, mode: 'chat', label: updated.defaultModel });
                  }
                }
              } else if (available.length > 0 && available.length <= 20) {
                nextGroups = automaticFavoritesForModels(available);
              }
              await savePreferencesFor(uid, {
                ...currentPrefs,
                favoriteGroups: nextGroups,
                selected: { modelId: updated.defaultModel, mode: 'chat' },
              });
            }
            await auditAdminAction('user.model-access', found.user.uid, uid, { modelGroupId: updated.modelGroupId || '', extraModelCount: updated.extraModels.length });
            return updated;
          });
          sendJson(res, 200, { user, revision: accountStore.data.revision });
          return;
        }
        if (req.method === 'PUT' && action === 'api-key') {
          const body = await readJson(req, 8 * 1024);
          const user = await runAdminMutation(async () => {
            const updated = await accountStore.setUserApiKey(uid, body?.apiKey ?? null);
            await auditAdminAction('user.api-key', found.user.uid, uid, { hasCustom: Boolean(updated.apiKey) });
            return updated;
          });
          sendJson(res, 200, { user, revision: accountStore.data.revision });
          return;
      ...
      if (req.method === 'GET' && (pathname === '/api/preferences' || pathname === '/api/settings')) {
        ...
        if (['user', 'guest'].includes(found.user.role) && availableModels.length > 0 && availableModels.length <= 20) {
          if (!userPreferences.favoriteGroups.length || isLegacyAutomaticFavorites(userPreferences.favoriteGroups)) {
            userPreferences = await savePreferencesFor(found.user.uid, {
              ...userPreferences,
              favoriteGroups: automaticFavoritesForModels(availableModels),
              selected: availableModels[0]?.modes?.includes('chat') ? { modelId: availableModels[0].id, mode: 'chat' } : userPreferences.selected,
            });
          }
        }
        sendJson(res, 200, preferencesPayload(userPreferences, availableModels));
        return;
      }
      ...
      if (req.method === 'PUT' && (pathname === '/api/preferences' || pathname === '/api/settings')) {
        ...
        const savedPreferences = await savePreferencesFor(found.user.uid, validatePreferences({
          ...body,
          favoriteMediaIds: body?.favoriteMediaIds === undefined ? currentPreferences.favoriteMediaIds : body.favoriteMediaIds,
          conversationTitleModel: body?.conversationTitleModel === undefined ? currentPreferences.conversationTitleModel : body.conversationTitleModel,
          readingMode: body?.readingMode === undefined ? currentPreferences.readingMode : body.readingMode,
          stream: body?.stream === undefined ? currentPreferences.stream : body.stream,
        }, models, { fallbackConversationTitleModel: body?.conversationTitleModel === undefined, existingPreferences: currentPreferences }));
        sendJson(res, 200, preferencesPayload(savedPreferences, models));
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/workflows/run') {
        ...
        const selectedImageModel = await newApi.requireModel(imageModel, 'image', undefined, { allowImageModeOverride: true });
      ...
      if (req.method === 'POST' && pathname === '/api/conversations/title') {
        ...
        const titleModels = await modelsForUser(found.user.uid);
        const titleModel = validateConversationTitleModel((await preferencesFor(found.user.uid)).conversationTitleModel, titleModels, { fallback: true });
        if (!titleModel) throw publicError(422, '当前没有可用于生成标题的对话模型', 'NO_CONVERSATION_TITLE_MODEL');
        await apiClientFor(found.user.uid).requireModel(titleModel, 'chat');
        let release;
        try { release = reserve(found.session.id); } catch (error) { throw error; }
        try {
          const result = await apiClientFor(found.user.uid).chat({
            model: titleModel,
            stream: false,
            maxTokens: 48,
            messages: [
          ...
          sendJson(res, 200, { title, model: titleModel });
        } finally { release(); }
        return;
      }
      ...
      if (req.method === 'PUT' && pathname === '/api/conversations') {
        ...
        const incomingFolders = Array.isArray(body?.folders) ? normalizeStoredFolders(body.folders) : undefined;
        const incomingDeletedIds = Array.isArray(body?.deletedIds)
          ? body.deletedIds.filter((id) => typeof id === 'string' && id.length > 0 && id.length <= 128)
          : [];
        const saved = await mergeAndSaveAdminConversations(incoming, incomingFolders, incomingDeletedIds);
      ...
      if (req.method === 'DELETE' && pathname.startsWith('/api/conversations/')) {
        ...
        if (!conversationId) throw publicError(400, '会话 ID 无效', 'INVALID_CONVERSATION_ID');
        const saved = await deleteAdminConversation(conversationId);
        sendJson(res, 200, saved);
        return;
      }
      ...
      if (req.method === 'DELETE' && pathname === '/api/conversations') {
        const found = requireAdministrator(req);
        requireSafeMutation(req, sessionManager, found);
        const saved = await clearAllAdminConversations();
        sendJson(res, 200, saved);
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/uploads') {
        ...
        const isRoleAttachment = req.headers['x-role-attachment'] === '1';
        const activeCountHeader = req.headers['x-active-attachments-count'];
        if (!isRoleAttachment) {
          const count = activeCountHeader !== undefined && Number.isInteger(Number(activeCountHeader))
            ? Number(activeCountHeader)
            : mediaStore.countUploads(mediaOwnerId).count;
          if (count >= MAX_ATTACHMENTS) throw publicError(413, `当前会话最多保留 ${MAX_ATTACHMENTS} 个待发送附件`, 'TOO_MANY_ATTACHMENTS');
        }
        const buffer = await readBody(req, MAX_INPUT_FILE_BYTES);
        const saved = await mediaStore.saveUpload(buffer, {
          sessionId: mediaOwnerId,
          fileName,
          declaredType,
          pending: !isRoleAttachment,
        });
        let extractedText = null;
        if (saved.mimeType === 'application/pdf') {
          try {
            extractedText = await pdfTextExtractor.extract(saved);
          } catch {
            extractedText = null;
          }
        }
        sendJson(res, 201, { attachment: saved, extractedText });
        return;
      }

      if (req.method === 'GET' && pathname === '/api/media/recent/neighbors') {
        const found = requireActiveAccount(req);
        const query = new URL(req.url, 'http://localhost').searchParams;
        const neighbors = mediaStore.recentImageNeighbors(found.user.uid, query.get('id'));
        if (!neighbors) throw publicError(404, '最近图片不存在或已过期', 'MEDIA_NOT_FOUND');
      ...
      if (req.method === 'GET' && pathname === '/api/media/recent') {
        ...
        const mediaOwnerId = found.user.uid;
        const query = new URL(req.url, 'http://localhost').searchParams;
        sendJson(res, 200, mediaStore.listRecent(mediaOwnerId, { page: query.get('page'), limit: query.get('limit') }));
        return;
      }
      ...
      if (req.method === 'GET' && pathname === '/api/media/favorites') {
        ...
        const query = new URL(req.url, 'http://localhost').searchParams;
        sendJson(res, 200, mediaStore.listOwned(preferences.favoriteMediaIds, found.user.uid, { page: query.get('page'), limit: query.get('limit') }));
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/chat') {
        ...
        if (body?.roleId !== undefined && body.roleId !== null && body.roleId !== '') {
          ...
          role = findRole(await rolesFor(found.user.uid), body.roleId);
          if (!role) throw publicError(400, '所选角色不存在或已被删除', 'ROLE_NOT_FOUND');
          systemPrompt = role.systemPrompt;
          systemPrompt = compileRoleSystemPrompt(role);
        }
        if (typeof body?.model !== 'string') throw publicError(400, '请选择模型', 'MODEL_REQUIRED');
        requireModelAccess(found.user.uid, body.model);
        const selectedModel = await apiClientFor(found.user.uid).requireModel(body.model, 'chat');
        let imageSize;
        ...
        if (body?.tools !== undefined) {
          if (!Array.isArray(body.tools) || body.tools.length > 32) throw publicError(400, '工具列表格式无效', 'INVALID_TOOLS');
          for (const t of body.tools) {
            if (!t || typeof t !== 'object' || t.type !== 'function' || !t.function?.name || typeof t.function.name !== 'string') {
              throw publicError(400, '工具定义格式无效', 'INVALID_TOOLS');
            }
          }
          tools = body.tools;
        }
        const upstreamMessages = await validateConversation(submittedMessages, mediaStore, pdfTextExtractor, mediaOwnerId, systemPrompt, contextLimit, role);
        ...
        const quota = reserveQuotaFor(found.user, quotaMode, quotaMode === 'image' ? imageCreditCost(body.model) : undefined);
        let release;
        ...
        try {
          const result = await apiClientFor(found.user.uid).chat({
            ...
            onToolCall: (toolCalls) => sendSse(res, 'tool_call', { toolCalls }),
            signal: controller.signal,
          });
          const images = [];
          ...
          if (stream) {
            if (result.toolCalls && result.toolCalls.length > 0) {
              sendSse(res, 'tool_call', { toolCalls: result.toolCalls });
            }
      ...
      if (req.method === 'POST' && pathname === '/api/tools/proxy') {
        const found = requireActiveAccount(req);
        requireSafeMutation(req, sessionManager, found);
        const body = await readJson(req, 128 * 1024);
        const targetUrl = typeof body?.url === 'string' ? body.url.trim() : '';
        if (!isSafePublicUrl(targetUrl)) {
          throw publicError(400, '目标地址无效或禁止访问内网地址', 'INVALID_TARGET_URL');
        }
        const method = typeof body?.method === 'string' ? body.method.toUpperCase() : 'GET';
        if (!['GET', 'POST', 'PUT'].includes(method)) {
          throw publicError(400, '不支持的请求方法', 'UNSUPPORTED_METHOD');
        }
        ...
        if (body?.headers && typeof body.headers === 'object') {
          for (const [key, val] of Object.entries(body.headers)) {
            const lower = key.toLowerCase();
            if (allowedHeaderNames.has(lower) && typeof val === 'string' && val.length < 2048) {
              forwardHeaders[key] = val;
            }
          }
        }
        let forwardBody = undefined;
        if (['POST', 'PUT'].includes(method) && body?.body !== undefined) {
          forwardBody = typeof body.body === 'string' ? body.body : JSON.stringify(body.body);
          if (forwardBody.length > 512 * 1024) throw publicError(413, '请求体过大', 'PAYLOAD_TOO_LARGE');
        }
        ...
        try {
          ...
          const raw = await readLimitedResponse(upstreamRes, 2 * 1024 * 1024);
          clearTimeout(timer);
          sendJson(res, 200, {
          ...
          clearTimeout(timer);
          if (controller.signal.aborted) throw publicError(504, '请求目标服务超时', 'UPSTREAM_TIMEOUT');
          throw publicError(502, '请求目标服务失败: ' + (error?.message || '未知错误'), 'PROXY_FAILED');
        }
        return;
      }
      ...
      if (req.method === 'POST' && (pathname === '/api/images/generations' || pathname === '/api/image')) {
        ...
        requireModelAccess(found.user.uid, body.model);
        const selectedModel = await apiClientFor(found.user.uid).requireModel(body.model, 'image', undefined, { allowImageModeOverride: true });
        ...
        const quota = reserveQuotaFor(found.user, 'image', imageCreditCost(body.model));
        let release;
        try { release = reserve(found.session.id); } catch (error) { quota.rollback(); throw error; }
        const controller = new AbortController();
        let clientCancelled = false;
        ...
        heartbeat?.unref?.();
        try {
          const generated = await apiClientFor(found.user.uid).generateImages({
            model: body.model,
            prompt: body.prompt,
            size,
            count,
            quality,
            allowImageModeOverride: true,
            signal: controller.signal,
          });
          const images = [];
      ...
      if (req.method === 'POST' && pathname === '/api/images/edits') {
        ...
        const prompt = requireImagePrompt(body);
        requireModelAccess(found.user.uid, body.model);
        const selectedModel = await apiClientFor(found.user.uid).requireModel(body.model, 'image');
        requirePromptWithinContext(prompt, modelContextLimit(await preferencesFor(found.user.uid), body.model));
        const imageOptions = selectedModel.imageOptions || {};
        ...
        heartbeat?.unref?.();
        try {
          const generated = await apiClientFor(found.user.uid).editImages({
            model: body.model,
            prompt: body.prompt,
            references,
            mask,
  ...
  server.on('upgrade', (req, socket, head) => {
    void proxyAdminUpgrade(req, socket, head);
  });

  return {
    server,
    stores: { accountStore, guestStore, preferencesStore, rolesStore, workflowsStore, mediaStore },
    async close() {
      for (const streams of sessionEventStreams.values()) for (const stream of [...streams]) closeSessionEventStream(stream);
