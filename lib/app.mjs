...
export function workflowImagePromptCandidates(chatText, workflowId = '') {
  const fullResponse = String(chatText || '').trim();
  if (!fullResponse) return { primary: '', fallback: '' };
  // 人设图角色的输出本身就是完整、可用的绘图提示词，绝不裁剪。
  if (workflowId !== 'epic-poster-v3') return { primary: fullResponse, fallback: '' };
...
export async function createChatApp({
  ...
  const builtInWorkflows = [
    { id: 'image-prompt-architect', name: '人设图生图', description: '先整理提示词，再生成成品图片', roleId: 'role-mrsmkx9c-2293', chatModel: 'claude-sonnet-4-5', imageModel: 'gpt-image-2', defaultSize: '1792x1024' },
    { id: 'epic-poster-v3', name: '史诗叙事海报生成', description: '将创意扩展为完整的叙事海报提示词并生成图片', roleId: 'role-mrqi90jc-7jmq', chatModel: 'claude-sonnet-4-5', imageModel: 'gpt-image-2', defaultSize: '1792x1024' },
  ];
  ...
  async function executeWorkflow({ workflow, userPrompt, imageModel, size, quality, userUid, sessionId, signal, onImage = () => {} }) {
    const chatQuota = accountStore.reserveQuota(userUid, 'chat');
    let imageQuota;
    try { imageQuota = accountStore.reserveQuota(userUid, 'image', imageCreditCost(imageModel)); } catch (error) { chatQuota.rollback(); throw error; }
    let release;
    try { release = reserve(sessionId); } catch (error) { chatQuota.rollback(); imageQuota.rollback(); throw error; }
    let imageStarted = false;
    try {
      // Reuse the same role-card lookup and system-prompt assembly as normal
      // role conversations.  Workflows always use the administrator's fixed
      // role card; ordinary users never receive its implementation details.
      const role = findRole(await rolesFor('00000'), workflow.roleId);
      if (!role) throw publicError(500, '打包工作流配置的角色不存在', 'WORKFLOW_ROLE_NOT_FOUND');
      const chatContextLimit = modelContextLimit(await preferencesFor(userUid), workflow.chatModel);
      ...
      const chat = await newApi.chat({ model: workflow.chatModel, messages: chatMessages, stream: false, signal });
      const promptCandidates = workflowImagePromptCandidates(chat.text, workflow.id);
      ...
      let generated;
      try {
        generated = await newApi.generateImages({ model: imageModel, prompt: promptCandidates.primary, size, count: 1, quality, signal });
      } catch (primaryError) {
        if (!promptCandidates.fallback) throw primaryError;
        requirePromptWithinContext(promptCandidates.fallback, imageContextLimit);
        generated = await newApi.generateImages({ model: imageModel, prompt: promptCandidates.fallback, size, count: 1, quality, signal });
      }
      const images = [];
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && (pathname === '/api/preferences' || pathname === '/api/settings')) {
        const found = requireActiveAccount(req);
        sendJson(res, 200, preferencesPayload(await preferencesFor(found.user.uid)));
        return;
      }
      ...
      if (req.method === 'GET' && pathname === '/api/workflows') {
        const found = requireActiveAccount(req);
        const admin = found.user.role === 'admin';
        sendJson(res, 200, { workflows: builtInWorkflows.map((workflow) => admin ? workflow : (({ id, name, description, defaultSize }) => ({ id, name, description, defaultSize }))(workflow)) });
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/workflows/run') {
        ...
        const workflow = builtInWorkflows.find((item) => item.id === body?.workflowId);
        const userPrompt = typeof body?.prompt === 'string' ? body.prompt.trim() : '';
        if (!workflow || !userPrompt || userPrompt.length > 12_000) throw publicError(400, '工作流输入无效', 'INVALID_WORKFLOW_INPUT');
        const imageModel = body?.imageModel === undefined ? workflow.imageModel : body.imageModel;
        ...
        await newApi.requireModel(workflow.chatModel, 'chat');
        const { count, size, quality } = imageRequestOptions({ size: body?.size, quality: body?.quality, count: 1 }, selectedImageModel);
  ...
  return {
    server,
    stores: { accountStore, preferencesStore, rolesStore, mediaStore },
    async close() {
