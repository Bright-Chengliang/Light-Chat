...
const OPC_UPSTREAM_HOST = '127.0.0.1';
...
export async function createChatApp({
  ...
  function opcPathFromRequest(req) {
    const parsed = new URL(req.url, 'http://localhost');
    if (!parsed.pathname.startsWith(`${OPC_PROXY_PREFIX}/`)) return null;
    return `${parsed.pathname.slice(OPC_PROXY_PREFIX.length) || '/'}${parsed.search}`;
  }

  function forwardOpcRequestHeaders(headers, { websocket = false } = {}) {
    const forwarded = {};
    for (const [name, value] of Object.entries(headers)) {
      const normalized = name.toLowerCase();
      if (
        HOP_BY_HOP_HEADERS.has(normalized)
        || ['host', 'cookie', 'authorization', 'x-csrf-token', 'accept-encoding'].includes(normalized)
      ) continue;
      forwarded[name] = value;
    }
    forwarded.host = `${OPC_UPSTREAM_HOST}:${opcPort}`;
    if (websocket) {
  ...
  function forwardOpcResponseHeaders(headers, { bodyChanged = false, websocket = false } = {}) {
    const forwarded = {};
    for (const [name, value] of Object.entries(headers)) {
      const normalized = name.toLowerCase();
      ...
      if (normalized === 'location' && typeof value === 'string') {
        try {
          const location = new URL(value, `http://${OPC_UPSTREAM_HOST}:${opcPort}`);
          if (location.hostname === OPC_UPSTREAM_HOST && location.port === String(opcPort)) {
            forwarded[name] = `${OPC_PROXY_PREFIX}${location.pathname}${location.search}${location.hash}`;
            continue;
          }
  ...
  function rewriteOpcBrowserBundle(source) {
    // OpenOPC is a standalone SPA. Its compiled bundle currently uses root
    // `/api/*` URLs and computes a WebSocket URL at `:8765`. Rewrite those
    // browser-only targets while proxying so all traffic remains on the
    // authenticated LightChat origin under `/opc`.
    return source
      .replaceAll('"/api/', '"/opc/api/')
      .replaceAll("'/api/", "'/opc/api/")
      .replaceAll('`/api/', '`/opc/api/')
      .replaceAll('window.location.hostname}:${window.location.port||"8765"}/ws', 'window.location.host}/opc/ws');
  }

  async function proxyOpcHttp(req, res, path) {
    await new Promise((resolve, reject) => {
      const upstream = httpRequest({
        host: OPC_UPSTREAM_HOST,
        port: opcPort,
        method: req.method,
        path,
        headers: forwardOpcRequestHeaders(req.headers),
      }, (upstreamRes) => {
        ...
        if (!isHtml && !isJavaScript) {
          res.writeHead(upstreamRes.statusCode || 502, forwardOpcResponseHeaders(upstreamRes.headers));
        ...
        upstreamRes.on('data', (chunk) => {
          size += chunk.length;
          if (size > 2 * 1024 * 1024) {
            upstreamRes.destroy(publicError(502, 'OPC 页面响应过大', 'OPC_RESPONSE_TOO_LARGE'));
            return;
          }
          chunks.push(chunk);
        });
        ...
        upstreamRes.once('end', () => {
          ...
          const body = isJavaScript ? rewriteOpcBrowserBundle(source) : source;
          res.writeHead(upstreamRes.statusCode || 502, forwardOpcResponseHeaders(upstreamRes.headers, { bodyChanged: true }));
          res.end(body);
          resolve();
        });
      });
      upstream.once('error', () => reject(publicError(502, 'OPC 服务暂时不可用', 'OPC_UNAVAILABLE')));
      req.once('aborted', () => upstream.destroy());
      req.pipe(upstream);
    });
  }

  function rejectOpcUpgrade(socket, status, message) {
    socket.write(`HTTP/1.1 ${status} ${message}\r\nConnection: close\r\nCache-Control: no-store\r\n\r\n`);
    socket.destroy();
  }

  function proxyOpcUpgrade(req, socket, head) {
    const path = opcPathFromRequest(req);
    try {
      if (!isAllowedHost(req.headers.host, allowedHosts, port)) throw publicError(421, '请求主机无效', 'HOST_REJECTED');
      if (path !== '/ws') throw publicError(404, '未找到该接口', 'NOT_FOUND');
      requireActiveAccount(req);
      requireSameOrigin(req, sessionManager);
    } catch (error) {
      rejectOpcUpgrade(socket, Number.isInteger(error?.status) ? error.status : 400, publicFailure(error).error);
      return;
    }
    ...
    const upstream = httpRequest({
      ...
      method: 'GET',
      path: '/ws',
      headers: forwardOpcRequestHeaders(req.headers, { websocket: true }),
    });
    upstream.once('error', () => rejectOpcUpgrade(socket, 502, 'OPC 服务暂时不可用'));
    upstream.once('response', (upstreamRes) => {
      const status = upstreamRes.statusCode || 502;
      const headers = forwardOpcResponseHeaders(upstreamRes.headers, { websocket: true });
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if ((req.method === 'GET' || req.method === 'HEAD') && pathname === OPC_PROXY_PREFIX) {
        requireAdministrator(req);
        const requestUrl = new URL(req.url, 'http://localhost');
        res.writeHead(303, { Location: `${OPC_PROXY_PREFIX}/${requestUrl.search}`, 'Cache-Control': 'no-store' });
        res.end();
        return;
      }
      if (pathname.startsWith(`${OPC_PROXY_PREFIX}/`)) {
        requireAdministrator(req);
        if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method || '')) requireSameOrigin(req, sessionManager);
        await proxyOpcHttp(req, res, opcPathFromRequest(req));
        return;
      }
  ...
  server.on('upgrade', (req, socket, head) => {
    void proxyOpcUpgrade(req, socket, head);
  });
