...
export async function createChatApp({
  ...
  async function modelsForUser(uid, { force = false } = {}) {
    if (uid === GUEST_UID) {
      if (!guestStore.endpoint) return [];
      const allowed = guestStore.allowedModelIds();
      if (allowed.size === 0) return [];
      const models = await guestApiClient().listModels({ force });
  ...
  function requireModelAccess(uid, modelId) {
    if (uid === GUEST_UID) {
      if (!guestStore.canUseModel(modelId)) throw publicError(403, '当前账号没有使用该模型的权限', 'MODEL_ACCESS_DENIED');
      return;
    }
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && pathname === '/api/guest/settings') {
        ...
        sendJson(res, 200, guestStore.publicSettings());
        return;
      }
      ...
      if (req.method === 'PUT' && pathname === '/api/guest/settings') {
        ...
        const saved = await guestStore.saveSettings({
      ...
      if (req.method === 'POST' && pathname === '/api/guest/models/preview') {
        ...
        requireSafeMutation(req, sessionManager, found);
        const body = await readJson(req, 16 * 1024);
        const endpoint = await normalizeGuestEndpoint(body?.endpoint, { allowPrivate: guestAllowPrivateEndpoints });
        if (!endpoint) throw publicError(400, '请先填写 API 服务端点', 'GUEST_ENDPOINT_REQUIRED');
        const apiKey = typeof body?.apiKey === 'string' && body.apiKey.trim()
          ? body.apiKey.trim().slice(0, 512)
          : guestStore.apiKeyValue;
        const previewClient = new NewApiClient({ apiKey, baseUrl: endpoint, fetchImpl });
        const models = await previewClient.listModels({ force: true });
      ...
      if (req.method === 'GET' && (pathname === '/api/preferences' || pathname === '/api/settings')) {
        ...
        if (['user', 'guest'].includes(found.user.role) && !userPreferences.favoriteGroups.length) {
