...
export async function createChatApp({
  ...
  async function mergeAndSaveAdminConversations(incoming, incomingFolders) {
    const task = adminConversationQueue.then(async () => {
      ...
      let folders = Array.isArray(incomingFolders)
        ? normalizeStoredFolders(incomingFolders)
        : (adminConversationData.folders || []);
      folders = ensureReferencedFolders(folders, merged);
  ...
  function apiClientFor(uid) {
    return uid === GUEST_UID ? guestApiClient() : newApi;
  }
  ...
  async function executeWorkflow({ workflow, userPrompt, imageModel, size, quality, userUid, sessionId, signal, onImage = () => {} }) {
    ...
    try {
      ...
      let generated;
      try {
        generated = await newApi.generateImages({ model: imageModel, prompt: promptCandidates.primary, size, count: 1, quality, forceTool: true, allowImageModeOverride: true, signal });
      } catch (primaryError) {
        if (!promptCandidates.fallback) throw primaryError;
        requirePromptWithinContext(promptCandidates.fallback, imageContextLimit);
        generated = await newApi.generateImages({ model: imageModel, prompt: promptCandidates.fallback, size, count: 1, quality, forceTool: true, allowImageModeOverride: true, signal });
      }
