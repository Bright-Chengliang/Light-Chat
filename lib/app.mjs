  ...
  workflows: [
    {
      ...
      nodes: [
        { id: 'architect', type: 'role', roleId: 'role-mrsmkx9c-2293', model: 'claude-sonnet-4-5', inputFrom: 'user', inputTemplate: '{{input}}', output: { mode: 'full' } },
        { id: 'image', type: 'image', model: 'gpt-image-2', promptFrom: 'architect', output: { mode: 'full' }, size: '1792x1024', quality: 'high', allowUserModelOverride: true, allowUserSizeOverride: true, allowUserQualityOverride: true },
      ],
    },
    {
      id: 'epic-poster-v3', name: '史诗叙事海报生成', description: '将创意扩展为完整的叙事海报提示词并生成图片', enabled: true,
      nodes: [
        { id: 'poster-master', type: 'role', roleId: 'role-mrqi90jc-7jmq', model: 'claude-sonnet-4-5', inputFrom: 'user', inputTemplate: '{{input}}', output: { mode: 'full' } },
        { id: 'image', type: 'image', model: 'gpt-image-2', promptFrom: 'poster-master', output: { mode: 'poster-chinese' }, size: '1792x1024', quality: 'high', allowUserModelOverride: true, allowUserSizeOverride: true, allowUserQualityOverride: true },
      ],
    },
  ],
});
...
function validatePreferences(input, models) {
  ...
  return { version: 1, favoriteGroups, selected, modelContextLimits: validateModelContextLimits(input.modelContextLimits, models) };
}
...
function workflowImageNode(workflow) { return workflow?.nodes?.at(-1)?.type === 'image' ? workflow.nodes.at(-1) : null; }
...
function validateWorkflowDefinitions(input, models, roleLibrary) {
  ...
  return list.map((workflow) => {
    ...
    const priorTextNodes = new Set(['user']);
    const normalizedNodes = nodes.map((node, index) => {
      const nodeId = workflowIdentifier(node?.id);
      const type = node?.type;
      if (!nodeId || nodeIds.has(nodeId) || !['role', 'temporary', 'image'].includes(type)) throw publicError(400, '工作流节点配置无效', 'INVALID_WORKFLOW_NODE');
      nodeIds.add(nodeId);
      if (type === 'image') {
        if (index !== nodes.length - 1) throw publicError(400, '生图节点必须位于工作流最后', 'INVALID_WORKFLOW_NODE');
        const model = modelMap.get(node?.model);
        const promptFrom = node?.promptFrom;
        const size = node?.size;
        const quality = node?.quality;
        if (!model?.modes.includes('image') || !priorTextNodes.has(promptFrom) || !ALLOWED_IMAGE_SIZES.has(size) || !ALLOWED_IMAGE_QUALITIES.has(quality)) throw publicError(400, '生图节点配置无效', 'INVALID_WORKFLOW_NODE');
        ...
        return {
          id: nodeId, type, model: model.id, promptFrom, output: validateWorkflowOutput(node.output), size, quality,
      ...
      if (index === nodes.length - 1) throw publicError(400, '工作流必须以生图节点结束', 'INVALID_WORKFLOW_NODE');
      const model = modelMap.get(node?.model);
      const inputFrom = node?.inputFrom;
      const inputTemplate = workflowText(node?.inputTemplate, 4_000) || '{{input}}';
      if (!model?.modes.includes('chat') || !priorTextNodes.has(inputFrom)) throw publicError(400, '文本节点配置无效', 'INVALID_WORKFLOW_NODE');
      const common = { id: nodeId, type, model: model.id, inputFrom, inputTemplate, output: validateWorkflowOutput(node.output) };
      if (type === 'role') {
    ...
    return { id, name, description, enabled: workflow?.enabled !== false, nodes: normalizedNodes };
  });
}
...
export async function createChatApp({
  ...
  const preferencesStore = new JsonStore(join(dataDir, 'preferences.json'), { version: 1, favoriteGroups: [], selected: null, modelContextLimits: {} });
  ...
  if (preferences?.version !== 1 || !Array.isArray(preferences.favoriteGroups)) {
    preferences = await preferencesStore.save({ version: 1, favoriteGroups: [], selected: null, modelContextLimits: {} });
  } else if (!preferences.modelContextLimits || typeof preferences.modelContextLimits !== 'object' || Array.isArray(preferences.modelContextLimits)) {
    preferences = await preferencesStore.save({ ...preferences, modelContextLimits: {} });
  }
  ...
  async function preferencesFor(uid) {
    ...
    const store = new JsonStore(join(dataDir, `preferences-${uid}.json`), { version: 1, favoriteGroups: [], selected: null, modelContextLimits: {} });
    let value = await store.load();
    if (value?.version !== 1 || !Array.isArray(value.favoriteGroups)) value = await store.save({ version: 1, favoriteGroups: [], selected: null, modelContextLimits: {} });
    if (!value.modelContextLimits || typeof value.modelContextLimits !== 'object' || Array.isArray(value.modelContextLimits)) value = await store.save({ ...value, modelContextLimits: {} });
  ...
  try {
    ...
    workflowData = { version: 1, workflows: validateWorkflowDefinitions(saved, upstreamModels, roleLibrary) };
  } catch {
  ...
  async function saveWorkflowDefinitions(value) {
    ...
    workflowData = await workflowsStore.save({ version: 1, workflows });
    return workflowData;
  }
  ...
  async function executeWorkflow({ workflow, userPrompt, imageModel, size, quality, userUid, sessionId, signal, onImage = () => {} }) {
    ...
    const textNodes = workflow.nodes.filter((node) => node.type === 'role' || node.type === 'temporary');
    if (!imageNode || !textNodes.length) throw publicError(500, '工作流缺少可执行节点', 'INVALID_WORKFLOW');
    const chatQuotas = [];
    let imageQuota;
    try {
    ...
    try {
      ...
      for (let index = 0; index < textNodes.length; index += 1) {
        ...
        const source = outputs.get(node.inputFrom);
      ...
      const promptCandidates = workflowTextCandidates(outputs.get(imageNode.promptFrom), imageNode.output);
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'PUT' && (pathname === '/api/preferences' || pathname === '/api/settings')) {
        ...
        const savedPreferences = await savePreferencesFor(found.user.uid, validatePreferences(body, models));
